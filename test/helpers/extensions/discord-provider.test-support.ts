import type { HanzoBotConfig } from "openclaw/plugin-sdk/discord";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: HanzoBotConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): HanzoBotConfig =>
  ({
    channels: {
      discord: {
        accounts: {
