...
Display HTML content on connected Hanzo Bot nodes (Mac app, iOS, Android).
...
```bash
cat ~/.hanzoai/bot.json | jq '.gateway.bind'
```
...
1. Check server bind: `cat ~/.hanzoai/bot.json | jq '.gateway.bind'`
