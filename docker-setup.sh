...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == BOT_GATEWAY_TOKEN=* ]]; then
      token="${line#BOT_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "BOT_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "BOT_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "BOT_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi
...
export BOT_EXTENSIONS="${BOT_EXTENSIONS:-}"
...
if [[ -z "${BOT_GATEWAY_TOKEN:-}" ]]; then
  ...
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    ...
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      BOT_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_workspace_mount="${BOT_WORKSPACE_DIR}:/home/node/.hanzo/bot/workspace"
...
if [[ "$IMAGE_NAME" == "openclaw:local" ]]; then
  ...
  docker build \
    ...
    --build-arg "BOT_EXTENSIONS=${BOT_EXTENSIONS}" \
...
# After fixing the config dir, only the Hanzo Bot metadata subdirectory
# (.openclaw/) inside the workspace gets chowned, not the user's project files.
docker compose "${COMPOSE_ARGS[@]}" run --rm --user root --entrypoint sh openclaw-cli -c \
  'find /home/node/.openclaw -xdev -exec chown node:node {} +; \
   [ -d /home/node/.hanzo/bot/workspace/.openclaw ] && chown -R node:node /home/node/.hanzo/bot/workspace/.openclaw || true'
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  # Enable sandbox in Hanzo Bot config.
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    echo "Docs: https://docs.hanzo.bot/gateway/sandboxing"
