...
If you believe you've found a security issue in HanzoBot, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [hanzo-bot/hanzo-bot](https://github.com/hanzo-bot/hanzo-bot)
- **macOS desktop app** — [hanzo-bot/hanzo-bot](https://github.com/hanzo-bot/hanzo-bot) (apps/macos)
- **iOS app** — [hanzo-bot/hanzo-bot](https://github.com/hanzo-bot/hanzo-bot) (apps/ios)
- **Android app** — [hanzo-bot/hanzo-bot](https://github.com/hanzo-bot/hanzo-bot) (apps/android)
- **ClawHub** — [hanzo-bot/clawhub](https://github.com/hanzo-bot/clawhub)
- **Trust and threat model** — [hanzo-bot/trust](https://github.com/hanzo-bot/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@hanzo-bot.ai](mailto:security@hanzo-bot.ai)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.hanzo-bot.ai).
...
- Tested version details (HanzoBot version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to HanzoBot's documented trust boundaries.
- For exposed-secret reports: proof the credential is HanzoBot-owned (or grants access to HanzoBot-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at HanzoBot. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

HanzoBot is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
HanzoBot does **not** model one gateway as a multi-tenant, adversarial user boundary.

- Authenticated Gateway callers are treated as trusted operators for that gateway instance.
- Session identifiers (`sessionKey`, session IDs, labels) are routing controls, not per-user authorization boundaries.
- If one operator can view data from another operator on the same gateway, that is expected in this trust model.
- HanzoBot can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need HanzoBot, use one VPS (or host/OS user boundary) per user.
...
- This is expected in HanzoBot's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of HanzoBot's trusted computing base for a gateway.
...
- Using HanzoBot in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.hanzo-bot`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not HanzoBot-owned and not granting access to HanzoBot-operated infrastructure/services) without demonstrated HanzoBot impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

HanzoBot security guidance assumes:

- The host where HanzoBot runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.hanzo-bot` state/config (including `hanzo-bot.json`) is effectively a trusted operator.
...
HanzoBot's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
HanzoBot separates routing from execution, but both remain inside the same operator trust boundary:
...
- Plugins can execute with the same OS privileges as the HanzoBot process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

HanzoBot uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/hanzo-bot` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/hanzo-bot` (or `hanzo-bot-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the HanzoBot-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use HanzoBot temp helpers (`resolvePreferredHanzoBotTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-hanzo-bot-dir.ts`
...
- `https://docs.hanzo-bot.ai/gateway/security`
...
HanzoBot's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `hanzo-bot gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - HanzoBot keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `hanzo-bot security audit` as dangerous findings.
...
HanzoBot requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running HanzoBot in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v hanzo-bot-data:/app/data \
  hanzo-bot/hanzo-bot:latest
```
