...
- **Core CLI and gateway** — [hanzoai/bot](https://github.com/hanzoai/bot)
- **macOS desktop app** — [hanzoai/bot](https://github.com/hanzoai/bot) (apps/macos)
- **iOS app** — [hanzoai/bot](https://github.com/hanzoai/bot) (apps/ios)
- **Android app** — [hanzoai/bot](https://github.com/hanzoai/bot) (apps/android)
- **ClawHub** — [openclaw/clawhub](https://github.com/openclaw/clawhub)
- **Trust and threat model** — [openclaw/trust](https://github.com/openclaw/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@hanzo.bot](mailto:security@hanzo.bot)** and we'll route it.
...
- Tested version details (Hanzo Bot version and/or commit SHA).
...
- Demonstrated impact tied to Hanzo Bot's documented trust boundaries.
- For exposed-secret reports: proof the credential is Hanzo Bot-owned (or grants access to Hanzo Bot-operated infrastructure/services).
...
Hanzo Bot does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Hanzo Bot can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need Hanzo Bot, use one VPS (or host/OS user boundary) per user.
...
- This is expected in Hanzo Bot's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of Hanzo Bot's trusted computing base for a gateway.
...
- Exposed secrets that are third-party/user-controlled credentials (not Hanzo Bot-owned and not granting access to Hanzo Bot-operated infrastructure/services) without demonstrated Hanzo Bot impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

Hanzo Bot security guidance assumes:

- The host where Hanzo Bot runs is within a trusted OS/admin boundary.
...
Hanzo Bot's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
Hanzo Bot separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when Hanzo Bot can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the Hanzo Bot process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

Hanzo Bot uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:
...
- Sandbox media validation allows absolute temp paths only under the Hanzo Bot-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use Hanzo Bot temp helpers (`resolvePreferredHanzo BotTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
...
Hanzo Bot's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.
...
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - Hanzo Bot keeps deployment flexibility by design and does not hard-forbid non-local setups.
