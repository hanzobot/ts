...
import { resolvePreferredHanzo BotTmpDir } from "openclaw/plugin-sdk/infra-runtime";
...
function resolveVoiceTtsConfig(params: { cfg: Hanzo BotConfig; override?: TtsConfig }): {
  cfg: Hanzo BotConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredHanzo BotTmpDir(), "discord-voice-"));
