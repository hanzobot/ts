...
- **Task tool** in the upstream spec == HanzoBot `sessions_spawn`
- **File I/O** == HanzoBot `read`/`write`
- **Remote fetch** == HanzoBot `web_fetch` (or `exec` with curl when POST is required)
