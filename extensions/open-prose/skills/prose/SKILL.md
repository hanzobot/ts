...
metadata: { "@hanzo/bot": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Hanzo Bot `sessions_spawn`
- **File I/O** == Hanzo Bot `read`/`write`
- **Remote fetch** == Hanzo Bot `web_fetch` (or `exec` with curl when POST is required)
