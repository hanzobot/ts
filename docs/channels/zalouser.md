...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Hanzo Bot.
...
- Install via CLI: `hanzo-bot plugins install @hanzo/bot-zalouser`
...
- On startup, Hanzo Bot resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Hanzo Bot stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Hanzo Bot state. Example:
...
- Hanzo Bot sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Hanzo Bot sends delivered + seen acknowledgements (best-effort).
...
- The channel now runs fully in Hanzo Bot without external CLI binaries.
