...
read_when:
  - Setting up Zalo Personal for HanzoBot
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside HanzoBot.
...
- On startup, HanzoBot resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
Accounts map to `zalouser` profiles in HanzoBot state. Example:
...
- HanzoBot sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, HanzoBot sends delivered + seen acknowledgements (best-effort).
...
- The channel now runs fully in HanzoBot without external CLI binaries.
