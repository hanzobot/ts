...
Tlon is a decentralized messenger built on Urbit. HanzoBot connects to your Urbit ship and can
...
```bash
hanzo-bot plugins install @hanzo/bot-tlon
```
...
By default, HanzoBot blocks private/internal hostnames and IP ranges for SSRF protection.
...
- Thread replies: if the inbound message is in a thread, HanzoBot replies in-thread.
