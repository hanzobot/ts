...
Matrix is an open, decentralized messaging protocol. HanzoBot connects as a Matrix **user**
...
```bash
hanzo-bot plugins install @hanzo/bot-matrix
```

Local checkout (when running from a git repo):

```bash
hanzo-bot plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `hanzo-bot plugins install @hanzo/bot-matrix`
   - From a local checkout: `hanzo-bot plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: HanzoBot calls the same
     login endpoint, stores the access token in `~/.hanzo-bot/credentials/matrix/credentials.json`,
...
- On first connection, HanzoBot requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  HanzoBot logs a warning.
...
`~/.hanzo-bot/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `hanzo-bot pairing list matrix`
  - `hanzo-bot pairing approve matrix <CODE>`
...
- On startup, HanzoBot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
