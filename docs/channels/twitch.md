...
read_when:
  - Setting up Twitch chat integration for HanzoBot
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. HanzoBot connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
hanzo-bot plugins install @hanzo/bot-twitch
```

Local checkout (when running from a git repo):

```bash
hanzo-bot plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "hanzo-bot", // Bot's Twitch account
