...
Hanzo Bot routes replies **back to the channel where a message came from**. The
...
Hanzo Bot infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Hanzo Bot still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Hanzo Bot would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
