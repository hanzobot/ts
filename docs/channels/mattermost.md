---
summary: "Mattermost bot setup and Hanzo Bot config"
read_when:
...
Hanzo Bot will offer the local install path automatically.
...
4. Configure Hanzo Bot and start the gateway.
...
Native slash commands are opt-in. When enabled, Hanzo Bot registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Hanzo Bot derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Hanzo Bot.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Hanzo Bot.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Hanzo Bot, not `404`.
...
- Approve via:
  - `hanzo-bot pairing list mattermost`
  - `hanzo-bot pairing approve mattermost <CODE>`
...
Use these target formats with `hanzo-bot message send` or cron/webhooks:
...
Hanzo Bot resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Hanzo Bot sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Hanzo Bot sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Hanzo Bot derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Hanzo Bot run on the same host/network namespace.
...
`hanzo-bot message send` and cron/webhook deliveries.
