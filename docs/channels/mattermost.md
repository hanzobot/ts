---
summary: "Mattermost bot setup and HanzoBot config"
read_when:
...
```bash
hanzo-bot plugins install @hanzo/bot-mattermost
```

Local checkout (when running from a git repo):

```bash
hanzo-bot plugins install ./extensions/mattermost
```
...
4. Configure HanzoBot and start the gateway.
...
Native slash commands are opt-in. When enabled, HanzoBot registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, HanzoBot derives one from gateway host/port + `callbackPath`.
...
- If `interactions.callbackBaseUrl` is omitted, HanzoBot derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and HanzoBot run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="hanzo-bot-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"hanzo-bot-mattermost-interactions",
