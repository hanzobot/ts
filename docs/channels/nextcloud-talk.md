...
```bash
hanzo-bot plugins install @hanzo/bot-nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
hanzo-bot plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "HanzoBot" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `hanzo-bot pairing list nextcloud-talk`
  - `hanzo-bot pairing approve nextcloud-talk <CODE>`
