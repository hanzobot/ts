...
- DM messages can carry `message_thread_id`; Hanzo Bot routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    Hanzo Bot can stream partial replies in real time:
    ...
    - DM: Hanzo Bot keeps the same preview message and performs a final edit in place (no second message)
    - group/topic: Hanzo Bot keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), Hanzo Bot falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, Hanzo Bot skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, Hanzo Bot automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, Hanzo Bot retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - Hanzo Bot pins the spawn confirmation message in-topic after a successful bind.
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, Hanzo Bot enqueues system events like:
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat, so only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, Hanzo Bot preserves the topic for both the approval prompt and the post-approval follow-up.
...
<AccordionGroup>
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, Hanzo Bot now retries these as recoverable network errors.
...
- Multi-account precedence:
  ...
  - If neither is set, Hanzo Bot falls back to the first normalized account ID and `hanzo-bot doctor` warns.
