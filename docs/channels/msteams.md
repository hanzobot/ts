...
```bash
hanzo-bot plugins install @hanzo/bot-msteams
```

Local checkout (when running from a git repo):

```bash
hanzo-bot plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
HanzoBot will offer the local install path automatically.
...
- Talk to HanzoBot via Teams DMs, group chats, or channels.
...
- On startup, HanzoBot resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
Before configuring HanzoBot, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                     |
   | ------------------ | --------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `hanzo-bot-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `hanzo-bot plugins install @hanzo/bot-msteams`
   - From a local checkout: `hanzo-bot plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "HanzoBot in Teams", "full": "HanzoBot in Teams" },
...
HanzoBot handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, HanzoBot only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/HanzoBotShared/` folder in the configured SharePoint site's default document library.
...
```bash
hanzo-bot message send --channel msteams \
...
hanzo-bot message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
hanzo-bot message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
hanzo-bot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
hanzo-bot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
