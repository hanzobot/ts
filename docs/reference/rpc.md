...
HanzoBot integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- HanzoBot owns lifecycle when `channels.signal.autoStart=true`.
...
- HanzoBot spawns `imsg rpc` as a child process (legacy iMessage integration).
