---
summary: "How HanzoBot builds prompt context and reports token usage + costs"
read_when:
...
HanzoBot tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

HanzoBot assembles its own system prompt on every run. It includes:
...
For images, HanzoBot downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from HanzoBot session logs.
...
`cacheWrite`. If pricing is missing, HanzoBot shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. HanzoBot can
...
Anthropic's 1M context window is currently beta-gated. HanzoBot can inject the
...
HanzoBot skips the `context-1m-*` beta header because Anthropic currently
