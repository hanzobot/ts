---
summary: "How Hanzo Bot builds prompt context and reports token usage + costs"
read_when:
...
Hanzo Bot tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Hanzo Bot assembles its own system prompt on every run. It includes:
...
For images, Hanzo Bot downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Hanzo Bot session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `hanzo-bot status --usage` and `hanzo-bot channels list` show
...
`cacheWrite`. If pricing is missing, Hanzo Bot shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Hanzo Bot can
...
Anthropic's 1M context window is currently beta-gated. Hanzo Bot can inject the
...
Hanzo Bot skips the `context-1m-*` beta header because Anthropic currently
