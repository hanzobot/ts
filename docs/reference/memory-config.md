...
summary: "Full configuration reference for Hanzo Bot memory search, embedding providers, QMD backend, hybrid search, and multimodal memory"
read_when:
...
This page covers the full configuration surface for Hanzo Bot memory search. For
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, Hanzo Bot retries with `qmd query`. If QMD fails or the binary is
  missing, Hanzo Bot automatically falls back to the builtin SQLite manager so
...
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  ...
  - If you want to pre-download models manually (and warm the same index Hanzo Bot
    ...
    Hanzo Bot's QMD state lives under your **state dir** (defaults to `~/.openclaw`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    Hanzo Bot uses:

    ```bash
    # Pick the same state dir Hanzo Bot uses
    STATE_DIR="${BOT_STATE_DIR:-$HOME/.openclaw}"
...
- If `memorySearch.multimodal.enabled = true`, Hanzo Bot also indexes supported image/audio files under `extraPaths` only. Default memory roots (`MEMORY.md`, `memory.md`, `memory/**/*.md`) stay Markdown-only.
- Symlinks are ignored (files or directories).

## Multimodal memory files (Gemini image + audio)

Hanzo Bot can index image and audio files from `memorySearch.extraPaths` when using Gemini embedding 2:
...
> Hanzo Bot will automatically reindex when it detects a model or dimension change.
