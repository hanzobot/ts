...
This document explains how HanzoBot manages sessions end-to-end:
...
HanzoBot is designed around a single **Gateway process** that owns session state.
...
HanzoBot persists sessions in two layers:
...
In `mode: "warn"`, HanzoBot reports potential evictions but does not mutate the store/files.
...
HanzoBot intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (HanzoBot consumes the events, but Pi decides when to compact).
...
HanzoBot also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, HanzoBot bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, HanzoBot leaves it alone.
...
- `hanzo-bot status` (CLI)
- `hanzo-bot sessions` / `sessions --json`
...
HanzoBot supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- HanzoBot strips/suppresses this in the delivery layer.

As of `2026.1.10`, HanzoBot also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
HanzoBot uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but HanzoBot’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `hanzo-bot status`.
