...
- With Anthropic API-key auth profiles, Hanzo Bot seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Hanzo Bot injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Hanzo Bot exposes dedicated cache-trace diagnostics for embedded agent runs.
