---
summary: "Deploy Hanzo Bot on Railway with one-click template"
read_when:
  - Deploying Hanzo Bot to Railway
  - You want a one-click cloud deploy with browser-based setup
title: "Railway"
---

Deploy Hanzo Bot on Railway with a one-click template and finish setup in your browser.
...
- `BOT_STATE_DIR=/data/.openclaw` (recommended)
- `BOT_WORKSPACE_DIR=/data/workspace` (recommended)
- `BOT_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Hanzo Bot state + workspace so you can migrate to another host without losing config or memory.
