---
summary: "Run HanzoBot in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want HanzoBot isolated from your main macOS environment
...
# HanzoBot on macOS VMs (Sandboxing)
...
Run HanzoBot in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume).
...
5. SSH in, install HanzoBot, configure channels
6. Done
...
The VM runs in the background. HanzoBot's daemon keeps the gateway running.
...
This is the killer feature of running on macOS. Use [BlueBubbles](https://bluebubbles.app) to add iMessage to HanzoBot.
...
```bash
lume stop hanzo-bot
lume clone hanzo-bot hanzo-bot-golden
```

Reset anytime:

```bash
lume stop hanzo-bot && lume delete hanzo-bot
lume clone hanzo-bot-golden hanzo-bot
lume run hanzo-bot --no-display
```
...
| VM IP not showing        | Wait for VM to fully boot, run `lume get hanzo-bot` again                           |
