---
summary: "Install Hanzo Bot declaratively with Nix"
read_when:
...
Install Hanzo Bot declaratively with **[nix-openclaw](https://github.com/openclaw/nix-openclaw)** -- a batteries-included Home Manager module.
...
When `BOT_NIX_MODE=1` is set (automatic with nix-openclaw), Hanzo Bot enters a deterministic mode that disables auto-install flows.

You can also set it manually:

```bash
export BOT_NIX_MODE=1
```

On macOS, the GUI app does not automatically inherit shell environment variables. Enable Nix mode via defaults instead:

```bash
defaults write ai.hanzo.bot.mac openclaw.nixMode -bool true
```
...
Hanzo Bot reads JSON5 config from `BOT_CONFIG_PATH` and stores mutable data in `BOT_STATE_DIR`. When running under Nix, set these explicitly to Nix-managed locations so runtime state and config stay out of the immutable store.

| Variable               | Default                                 |
| ---------------------- | --------------------------------------- |
| `BOT_HOME`        | `HOME` / `USERPROFILE` / `os.homedir()` |
| `BOT_STATE_DIR`   | `~/.openclaw`                           |
| `BOT_CONFIG_PATH` | `$BOT_STATE_DIR/openclaw.json`     |
