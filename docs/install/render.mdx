...
Deploy HanzoBot on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/hanzo-bot/hanzo-bot)
...
repository configures everything needed to run HanzoBot:
...
The web dashboard is available at `https://<your-service>.onrender.com/hanzo-bot`.
...
If you use the original HanzoBot repository, Render will not auto-deploy your HanzoBot. To update it, run a manual Blueprint sync from the dashboard.
...
For HanzoBot, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any HanzoBot host.
