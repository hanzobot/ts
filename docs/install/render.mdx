---
summary: "Deploy Hanzo Bot on Render with Infrastructure-as-Code"
read_when:
  - Deploying Hanzo Bot to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Hanzo Bot on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/hanzoai/bot)
...
repository configures everything needed to run Hanzo Bot:
...
If you use the original Hanzo Bot repository, Render will not auto-deploy your Hanzo Bot. To update it, run a manual Blueprint sync from the dashboard.
...
For Hanzo Bot, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Hanzo Bot host.
