...
read_when:
  - You want to understand `hanzo.bot/install.sh`
...
| [`install-cli.sh`](#install-clish) | macOS / Linux / WSL  | Installs Node + Hanzo Bot into a local prefix (`~/.openclaw`). No root required.              |
...
<Steps>
  ...
  <Step title="Ensure Node.js 24 by default">
    Checks Node version and installs Node 24 if needed (Homebrew on macOS, NodeSource setup scripts on Linux apt/dnf/yum). Hanzo Bot still supports Node 22 LTS, currently `22.16+`, for compatibility.
  </Step>
...
| `BOT_VERSION=latest\|next\|main\|<semver>\|<spec>` | npm version, dist-tag, or package spec        |
