---
summary: "Optional Docker-based setup and onboarding for HanzoBot"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run HanzoBot on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full HanzoBot in Docker)
...
- `BOT_IMAGE` — use a remote image instead of building locally (e.g. `ghcr.io/hanzo-bot/hanzo-bot:latest`)
...
- Need the URL again? Run `docker compose run --rm hanzo-bot-cli dashboard --no-open`.
...
- If `Dockerfile.sandbox` is missing, the script prints a warning and continues;
  build `hanzo-bot-sandbox:bookworm-slim` with `scripts/sandbox-setup.sh` if
  needed.
...
```bash
docker compose run -T --rm hanzo-bot-cli gateway probe
docker compose run -T --rm hanzo-bot-cli devices list --json
```
...
`hanzo-bot-cli` uses `network_mode: "service:hanzo-bot-gateway"` so CLI commands can
...
network path instead of the bundled `hanzo-bot-cli` service.

To reduce impact if the CLI process is compromised, the compose config drops
`NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on `hanzo-bot-cli`.
...
- [GitHub Container Registry package](https://github.com/hanzo-bot/hanzo-bot/pkgs/container/hanzo-bot)

Use image name `ghcr.io/hanzo-bot/hanzo-bot` (not similarly named Docker Hub
images).
...
- `org.opencontainers.image.source=https://github.com/hanzo-bot/hanzo-bot`
- `org.opencontainers.image.url=https://hanzo-bot.ai`
- `org.opencontainers.image.documentation=https://docs.hanzo-bot.ai/install/docker`
- `org.opencontainers.image.licenses=MIT`
- `org.opencontainers.image.title=HanzoBot`
- `org.opencontainers.image.description=HanzoBot gateway and CLI runtime container image`
...
```bash
export BOT_IMAGE="ghcr.io/hanzo-bot/hanzo-bot:latest"
./docker-setup.sh
```

The script detects that `BOT_IMAGE` is not the default `hanzo-bot:local` and
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/hanzo-bot/hanzo-bot/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
```
...
See [`ClawDock` Helper README](https://github.com/hanzo-bot/hanzo-bot/blob/main/scripts/shell-helpers/README.md) for details.

### Manual flow (compose)

```bash
docker build -t hanzo-bot:local -f Dockerfile .
docker compose run --rm hanzo-bot-cli onboard
docker compose up -d hanzo-bot-gateway
```
...
```bash
docker compose run --rm hanzo-bot-cli dashboard --no-open
docker compose run --rm hanzo-bot-cli devices list
docker compose run --rm hanzo-bot-cli devices approve <requestId>
```
...
`hanzo-bot-gateway` and `hanzo-bot-cli` by generating `docker-compose.extra.yml`.
...
```bash
docker compose run --rm hanzo-bot-cli \
...
`/home/node/.hanzo-bot`, make sure your host bind mounts are owned by uid 1000.

Example (Linux host):

```bash
sudo chown -R 1000:1000 /path/to/hanzo-bot-config /path/to/hanzo-bot-workspace
```
...
```bash
docker compose run --rm hanzo-bot-cli channels login
```

Telegram (bot token):

```bash
docker compose run --rm hanzo-bot-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):

```bash
docker compose run --rm hanzo-bot-cli channels add --channel discord --token "<token>"
```
...
background. In plain terms: Docker keeps checking if HanzoBot is still
...
```bash
docker compose exec hanzo-bot-gateway node dist/index.js health --token "$BOT_GATEWAY_TOKEN"
```
...
```bash
docker compose run --rm hanzo-bot-cli config set gateway.mode local
docker compose run --rm hanzo-bot-cli config set gateway.bind lan
docker compose run --rm hanzo-bot-cli devices list --url ws://127.0.0.1:18789
```
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  HanzoBot auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `hanzo-bot sandbox recreate ...` command.
...
This builds `hanzo-bot-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `hanzo-bot-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "hanzo-bot-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
This builds `hanzo-bot-sandbox-browser:bookworm-slim` using
...
- Browser containers default to a dedicated Docker network (`hanzo-bot-sandbox-browser`) instead of global `bridge`.
...
- noVNC observer access is password-protected by default; HanzoBot provides a short-lived observer token URL that serves a local bootstrap page and keeps the password in URL fragment (instead of URL query).
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-hanzo-bot-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-hanzo-bot-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-hanzo-bot-sbx" } },
    },
  },
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/hanzo-bot/hanzo-bot/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: HanzoBot runs commands with `sh -lc` (login shell), which
