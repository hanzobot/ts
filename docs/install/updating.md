---
summary: "Updating HanzoBot safely (global install or source), plus rollback strategy"
read_when:
  - Updating HanzoBot
...
HanzoBot is moving fast (pre “1.0”). Treat updates like shipping infra: update → run checks → restart (or use `hanzo-bot update`, which restarts) → verify.
...
- `stable`: when a new version is seen, HanzoBot waits `stableDelayHours` and then applies a deterministic per-install jitter in `stableJitterHours` (spread rollout).
...
- `pnpm build` matters when you run the packaged `hanzo-bot` binary ([`hanzo-bot.mjs`](https://github.com/hanzo-bot/hanzo-bot/blob/main/hanzo-bot.mjs)) or use Node to run `dist/`.
...
- Detect and migrate older gateway services (launchd/systemd; legacy schtasks) to current HanzoBot services.
...
- macOS launchd (app-bundled LaunchAgent): `launchctl kickstart -k gui/$UID/ai.hanzo.bot.gateway` (use `ai.hanzo.bot.<profile>`; legacy `com.hanzo-bot.*` still works)
