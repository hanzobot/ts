---
summary: "Uninstall Hanzo Bot completely (CLI, service, state, workspace)"
read_when:
  - You want to remove Hanzo Bot from a machine
...
```bash
rm -rf /Applications/Hanzo Bot.app
```
...
Default label is `ai.hanzo.bot.gateway` (or `ai.hanzo.bot.<profile>`; legacy `com.openclaw.*` may still exist):
...
If you used a profile, replace the label and plist name with `ai.hanzo.bot.<profile>`. Remove any legacy `com.openclaw.*` plists if present.
...
Default task name is `Hanzo Bot Gateway` (or `Hanzo Bot Gateway (<profile>)`).
The task script lives under your state dir.

```powershell
schtasks /Delete /F /TN "Hanzo Bot Gateway"
...
If you used `https://hanzo.bot/install.sh` or `install.ps1`, the CLI was installed with `npm install -g @hanzo/bot@latest`.
