---
summary: "Deploy Hanzo Bot on Northflank with one-click template"
read_when:
  - Deploying Hanzo Bot to Northflank
  - You want a one-click cloud deploy with browser-based setup
title: "Northflank"
---

Deploy Hanzo Bot on Northflank with a one-click template and finish setup in your browser.
...
1. Click [Deploy Hanzo Bot](https://northflank.com/stacks/deploy-openclaw) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy Hanzo Bot now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the Hanzo Bot template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the Hanzo Bot service.
8. Open the public Hanzo Bot URL and complete setup at `/setup`.
9. Open the Control UI at `/openclaw`.

## What you get

- Hosted Hanzo Bot Gateway + Control UI
