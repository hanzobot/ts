...
HanzoBot ships three update channels:
...
When you **explicitly** switch channels with `--channel`, HanzoBot also aligns
the install method:

- `dev` ensures a git checkout (default `~/hanzo-bot`, override with `BOT_GIT_DIR`),
...
When you switch channels with `hanzo-bot update`, HanzoBot also syncs plugin sources:
