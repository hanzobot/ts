...
- Same path: run the app from a fixed location (for Hanzo Bot, `dist/Hanzo Bot.app`).
...
```bash
sudo tccutil reset Accessibility ai.hanzo.bot.mac
sudo tccutil reset ScreenCapture ai.hanzo.bot.mac
...
Workaround: move files into the Hanzo Bot workspace (`~/.hanzo/bot/workspace`) if you want to avoid per-folder grants.
