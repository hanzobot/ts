...
- You can still use the CLI flow in [Gateway health](/gateway/health) (`hanzo-bot status`, `hanzo-bot status --deep`, `hanzo-bot health --json`) and tail `/tmp/hanzoai/bot-*.log` for `web-heartbeat` / `web-reconnect`.
