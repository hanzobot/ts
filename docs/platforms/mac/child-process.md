...
service via the external `hanzo-bot` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.hanzo.bot.gateway`
  (or `ai.hanzo.bot.<profile>` when using `--profile`/`BOT_PROFILE`; legacy `com.hanzo-bot.*` is supported).
