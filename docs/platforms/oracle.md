---
summary: "HanzoBot on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up HanzoBot on Oracle Cloud
  - Looking for low-cost VPS hosting for HanzoBot
  - Want 24/7 HanzoBot on a small server
title: "Oracle Cloud"
---

# HanzoBot on Oracle Cloud (OCI)

## Goal

Run a persistent HanzoBot Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for HanzoBot (especially if you already have an OCI account), but it comes with tradeoffs:
...
```bash
# Set hostname
sudo hostnamectl set-hostname hanzo-bot
...
sudo tailscale up --ssh --hostname=hanzo-bot
```

This enables Tailscale SSH, so you can connect via `ssh hanzo-bot` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@hanzo-bot` (or use the Tailscale IP).
...
systemctl --user restart hanzo-bot-gateway
```
...
# Check daemon status
systemctl --user status hanzo-bot-gateway
...
```
https://hanzo-bot.<tailnet-name>.ts.net/
```
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `hanzo-bot security audit`, and verify you aren’t accidentally listening on public interfaces.
...
ssh -L 18789:127.0.0.1:18789 ubuntu@hanzo-bot
```
...
# Re-authenticate
sudo tailscale up --ssh --hostname=hanzo-bot --reset
```
...
journalctl --user -u hanzo-bot-gateway -n 50
```
