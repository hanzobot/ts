---
summary: "HanzoBot macOS companion app (menu bar + gateway broker)"
read_when:
...
# HanzoBot macOS Companion (menu bar + gateway broker)

The macOS app is the **menu‑bar companion** for HanzoBot. It owns permissions,
...
- Installs the global CLI (`hanzo-bot`) via npm/pnpm on request (bun not recommended for the Gateway runtime).
...
(or `ai.hanzo.bot.<profile>` when using `--profile`/`BOT_PROFILE`; legacy `com.hanzo-bot.*` still unloads).
...
The app registers the `hanzo-bot://` URL scheme for local actions.
...
1. Install and launch **HanzoBot.app**.
...
Avoid putting your HanzoBot state dir in iCloud or other cloud-synced folders.
