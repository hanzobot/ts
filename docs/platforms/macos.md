...
(or `ai.hanzo.bot.<profile>` when using `--profile`/`BOT_PROFILE`; legacy `com.openclaw.*` still unloads).
...
Avoid putting your Hanzo Bot state dir in iCloud or other cloud-synced folders.
