...
- You can run the CLI (`hanzo-bot`) on the gateway machine (or via SSH).
...
- Set `gateway.bind: "tailnet"` in `~/.hanzo-bot/hanzo-bot.json` on the gateway host.
...
1. Set up a DNS-SD zone (example `hanzo-bot.internal.`) on the gateway host and publish `_hanzo-bot-gw._tcp` records.
...
```bash
hanzo-bot nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__bot__/canvas/"}'
```
