...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in HanzoBot
...
Running HanzoBot via Node with `tsx` fails at startup with:

```
[hanzo-bot] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `hanzo-bot status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch hanzo-bot.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node hanzo-bot.mjs status` works on Node 25.
