...
- `gateway`/`node` approvals are controlled by `~/.hanzo/bot/exec-approvals.json`.
...
- Hanzo Bot sets `BOT_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Hanzo Bot prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`hanzo-bot security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `hanzo-bot doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
