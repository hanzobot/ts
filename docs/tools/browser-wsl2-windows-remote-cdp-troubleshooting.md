...
read_when:
  - Running Hanzo Bot Gateway in WSL2 while Chrome lives on Windows
...
- Hanzo Bot Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/hanzoai/bot/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Hanzo Bot and Chrome are on the same machine
...
- Hanzo Bot points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Hanzo Bot is not the problem yet.
...
Fix that before touching Hanzo Bot config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Hanzo Bot at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Hanzo Bot to succeed
...
- `hanzo-bot browser tabs` returns the target
...
3. Hanzo Bot config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Hanzo Bot config or Control UI auth
