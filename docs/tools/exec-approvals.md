...
- For shell scripts and direct interpreter/runtime file invocations, Hanzo Bot also tries to bind
...
- allowlist entries live in host-local `~/.hanzo/bot/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `hanzo-bot approvals allowlist ...`).
- `hanzo-bot security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `hanzo-bot doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
These clients are opt-in. If a channel does not have exec approvals enabled, Hanzo Bot does not treat
...
topics, Hanzo Bot preserves the topic for the approval prompt and the post-approval follow-up.
