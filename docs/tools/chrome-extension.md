---
summary: "Chrome extension: let HanzoBot drive your existing Chrome tab"
read_when:
...
The HanzoBot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate hanzo-bot-managed Chrome profile.
...
HanzoBot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the HanzoBot release (npm package) as static files. There is no separate “build” step.

After upgrading HanzoBot:

- Re-run `hanzo-bot browser extension install` to refresh the installed files under your HanzoBot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

HanzoBot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want HanzoBot to control.
...
- `ON`: attached; HanzoBot can drive that tab.
...
Debugging: `hanzo-bot sandbox explain`
...
`hanzo-bot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `hanzo-bot browser extension install` first to copy the extension to a stable location under your HanzoBot state directory.
...
- **This is not isolated** like the dedicated hanzo-bot-managed profile.
