...
Hanzo Bot can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `hanzo-bot configure --section web` enables the bundled Firecrawl plugin automatically.
...
Hanzo Bot always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
