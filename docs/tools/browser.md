...
title: "Browser (HanzoBot-managed)"
---

# Browser (hanzo-bot-managed)

HanzoBot can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `hanzo-bot` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The default `chrome` profile uses the **system default Chromium browser** via the
  extension relay; switch to `hanzo-bot` for the isolated managed browser.

## What you get

- A separate browser profile named **hanzo-bot** (orange accent by default).
- Deterministic tab control (list/open/focus/close).
- Agent actions (click/type/drag/select), snapshots, screenshots, PDFs.
- Optional multi-profile support (`hanzo-bot`, `work`, `remote`, ...).
...
## Profiles: `hanzo-bot` vs `chrome`

- `hanzo-bot`: managed, isolated browser (no extension required).
- `chrome`: extension relay to your **system browser** (requires the HanzoBot
  extension to be attached to a tab).

Set `browser.defaultProfile: "hanzo-bot"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.hanzo-bot/hanzo-bot.json`.
...
- Default profile is `hanzo-bot` (HanzoBot-managed standalone browser). Use `defaultProfile: "chrome"` to opt into the Chrome extension relay.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome → Brave → Edge → Chromium → Chrome Canary.
- Local `hanzo-bot` profiles auto-assign `cdpPort`/`cdpUrl` — set those only for remote CDP.

## Use Brave (or another Chromium-based browser)

If your **system default** browser is Chromium-based (Chrome/Brave/Edge/etc),
HanzoBot uses it automatically. Set `browser.executablePath` to override
auto-detection:
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, HanzoBot will not launch a local browser.
...
HanzoBot preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, HanzoBot can
...
CDP endpoints over HTTPS. You can point a HanzoBot browser profile at a
...
- If browser control is enabled and no auth is configured, HanzoBot auto-generates `gateway.auth.token` on startup and persists it to config.
...
HanzoBot supports multiple named profiles (routing configs). Profiles can be:

- **hanzo-bot-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **extension relay**: your existing Chrome tab(s) via the local relay + Chrome extension

Defaults:

- The `hanzo-bot` profile is auto-created if missing.
...
HanzoBot can also drive **your existing Chrome tabs** (no separate “hanzo-bot” Chrome instance) via a local CDP relay + a Chrome extension.
...
- You click the **HanzoBot Browser Relay** extension icon on a tab to attach (it does not auto-attach).
...
When launching locally, HanzoBot picks the first available:
...
- `x-hanzo-bot-password: <gateway password>` or HTTP Basic auth with that password

### Playwright requirement

Some features (navigate/act/AI snapshot/role snapshot, element screenshots, PDF) require
Playwright. If Playwright isn’t installed, those endpoints return a clear 501
error. ARIA snapshots and basic screenshots still work for hanzo-bot-managed Chrome.
For the Chrome extension relay driver, ARIA snapshots and screenshots require Playwright.

If you see `Playwright is not available in this gateway build`, install the full
Playwright package (not `playwright-core`) and restart the gateway, or reinstall
HanzoBot with browser support.
...
- `hanzo-bot browser download e12 report.pdf`
- `hanzo-bot browser waitfordownload report.pdf`
- `hanzo-bot browser upload /tmp/hanzo-bot/uploads/file.pdf`
...
- `hanzo-bot browser set headers --headers-json '{"X-Debug":"1"}'`
...
- Download and trace output paths are constrained to HanzoBot temp roots:
  - traces: `/tmp/hanzo-bot` (fallback: `${os.tmpdir()}/hanzo-bot`)
  - downloads: `/tmp/hanzo-bot/downloads` (fallback: `${os.tmpdir()}/hanzo-bot/downloads`)
- Upload paths are constrained to an HanzoBot temp uploads root:
  - uploads: `/tmp/hanzo-bot/uploads` (fallback: `${os.tmpdir()}/hanzo-bot/uploads`)
- `upload` can also set file inputs directly via `--input-ref` or `--element`.
- `snapshot`:
  - `--format ai` (default when Playwright is installed): returns an AI snapshot with numeric refs (`aria-ref="<n>"`).
  - `--format aria`: returns the accessibility tree (no refs; inspection only).
  - `--efficient` (or `--mode efficient`): compact role snapshot preset (interactive + compact + depth + lower maxChars).
  - Config default (tool/CLI only): set `browser.snapshotDefaults.mode: "efficient"` to use efficient snapshots when the caller does not pass a mode (see [Gateway configuration](/gateway/configuration#browser-hanzo-bot-managed-browser)).
...
HanzoBot supports two “snapshot” styles:
...
- `browser` accepts:
  - `profile` to choose a named browser profile (hanzo-bot, chrome, or remote CDP).
