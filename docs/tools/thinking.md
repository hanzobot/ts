...
- Provider notes:
  ...
  - Moonshot (`moonshot/*`) maps `/think off` to `thinking: { type: "disabled" }` and any non-`off` level to `thinking: { type: "enabled" }`. When thinking is enabled, Moonshot only accepts `tool_choice` `auto|none`; Hanzo Bot normalizes incompatible values to `auto`.
...
- Hanzo Bot resolves fast mode in this order:
...
- For `openai-codex/*`, fast mode applies the same low-latency profile on Codex Responses. Hanzo Bot keeps one shared `/fast` toggle across both auth paths.
...
- Anthropic fast mode is API-key only. Hanzo Bot skips Anthropic service-tier injection for Claude setup-token / OAuth auth and for non-Anthropic proxy base URLs.
