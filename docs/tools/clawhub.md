...
If you want to add new capabilities to your Hanzo Bot agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
By default, the CLI installs skills into `./skills` under your current working directory. If an Hanzo Bot workspace is configured, `clawhub` falls back to that workspace unless you override `--workdir` (or `CLAWHUB_WORKDIR`). Hanzo Bot loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.hanzo/bot/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Hanzo Bot how to perform a
...
Interested in becoming a moderator? Ask in the Hanzo Bot Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Hanzo Bot workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/openclaw/skills` and `~/.hanzo/bot/skills`). This is designed to find older skill installs without extra flags.
