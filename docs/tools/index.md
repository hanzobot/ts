---
summary: "Agent tool surface for HanzoBot (browser, canvas, nodes, message, cron) replacing legacy `hanzo-bot-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `hanzo-bot-*` skills
title: "Tools"
---

# Tools (HanzoBot)

HanzoBot exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `hanzo-bot-*` skills: the tools are typed, no shelling,
...
- If `tools.allow` only references unknown or unloaded plugin tool names, HanzoBot logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:hanzo-bot`: all built-in HanzoBot tools (excludes provider plugins)
...
HanzoBot tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `hanzo-bot configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated HanzoBot-managed browser.
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, HanzoBot uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- `sessions_spawn` supports inline file attachments for subagent runtime only (ACP rejects them). Each attachment has `name`, `content`, and optional `encoding` (`utf8` or `base64`) and `mimeType`. Files are materialized into the child workspace at `.hanzo-bot/attachments/<uuid>/` with a `.manifest.json` metadata file. The tool returns a receipt with `count`, `totalBytes`, per file `sha256`, and `relDir`. Attachment content is automatically redacted from transcript persistence.
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, HanzoBot clamps `tools.sessions.visibility` to `tree`.
