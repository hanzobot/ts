---
title: Lobster
summary: "Typed workflow runtime for HanzoBot with resumable approval gates."
description: Typed workflow runtime for HanzoBot — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets HanzoBot run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: HanzoBot runs one Lobster tool call and gets a structured result.
...
HanzoBot launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | hanzo-bot.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
hanzo-bot.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In HanzoBot tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the HanzoBot Gateway (see the [Lobster repo](https://github.com/hanzo-bot/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), HanzoBot keeps core tools enabled. To restrict core
...
- **No secrets** — Lobster doesn't manage OAuth; it calls HanzoBot tools that do.
