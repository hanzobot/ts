...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `hanzo-bot sandbox explain`.
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${BOT_STATE_DIR:-$HOME/.openclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
