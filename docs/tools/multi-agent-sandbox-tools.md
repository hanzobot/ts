...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `hanzo-bot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `hanzo-bot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=hanzo-bot-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${BOT_STATE_DIR:-$HOME/.hanzo-bot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
