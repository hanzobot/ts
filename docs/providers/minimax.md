---
summary: "Use MiniMax M2.5 in HanzoBot"
read_when:
  - You want MiniMax models in HanzoBot
...
- **Compatibility:** HanzoBot still accepts legacy `MiniMax-M2.5-Lightning` configs, but prefer
...
```bash
hanzo-bot plugins enable minimax-portal-auth  # skip if already loaded.
hanzo-bot gateway restart  # restart if gateway is already running
hanzo-bot onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/hanzo-bot/hanzo-bot/tree/main/extensions/minimax-portal-auth) for details.
