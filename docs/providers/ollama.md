---
summary: "Run Hanzo Bot with Ollama (cloud and local models)"
read_when:
  - You want to run Hanzo Bot with cloud or local models via Ollama
...
Hanzo Bot currently suggests:

- local default: `glm-4.7-flash`
- cloud defaults: `kimi-k2.5:cloud`, `minimax-m2.5:cloud`, `glm-5:cloud`

5. If you prefer manual setup, enable Ollama for Hanzo Bot directly (any value works; Ollama doesn't require a real key):
...
When you set `OLLAMA_API_KEY` (or an auth profile) and **do not** define `models.providers.ollama`, Hanzo Bot discovers models from the local Ollama instance at `http://127.0.0.1:11434`:
...
- Sets `maxTokens` to the default Ollama max-token cap used by Hanzo Bot
- Sets all costs to `0`
...
If `OLLAMA_API_KEY` is set, you can omit `apiKey` in the provider entry and Hanzo Bot will fill it for availability checks.
...
Hanzo Bot treats models with names such as `deepseek-r1`, `reasoning`, or `think` as reasoning-capable by default:
...
Hanzo Bot's Ollama integration uses the **native Ollama API** (`/api/chat`) by default, which fully supports streaming and tool calling simultaneously. No special configuration is needed.
...
For auto-discovered models, Hanzo Bot uses the context window reported by Ollama when available, otherwise it falls back to the default Ollama context window used by Hanzo Bot. You can override `contextWindow` and `maxTokens` in explicit provider config.
