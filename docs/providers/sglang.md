---
summary: "Run Hanzo Bot with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Hanzo Bot against a local SGLang server
...
Hanzo Bot can connect to SGLang using the `openai-completions` API.

Hanzo Bot can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Hanzo Bot will query:
