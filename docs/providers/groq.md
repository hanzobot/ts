...
read_when:
  - You want to use Groq with Hanzo Bot
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Hanzo Bot connects
...
media-understanding provider, Hanzo Bot uses Groq's `whisper-large-v3-turbo`
...
available to that process (for example, in `~/.hanzo/bot/.env` or via
`env.shellEnv`).

## Available models

Groq's model catalog changes frequently. Run `hanzo-bot models list | grep groq`
