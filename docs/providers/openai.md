---
summary: "Use OpenAI via API keys or Codex subscription in HanzoBot"
read_when:
  - You want to use OpenAI models in HanzoBot
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like HanzoBot.
...
OpenAI API usage. HanzoBot forwards both through the `openai/*` Responses path.
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. HanzoBot
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

### Transport default

HanzoBot uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), HanzoBot also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. HanzoBot enables it by default for
...
HanzoBot, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
`baseUrl` on `api.openai.com`), HanzoBot now auto-enables OpenAI server-side
