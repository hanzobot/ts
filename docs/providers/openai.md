...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Hanzo Bot.
...
OpenAI API usage. Hanzo Bot forwards both through the `openai/*` Responses path.
Hanzo Bot intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

Hanzo Bot does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in Hanzo Bot.
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. Hanzo Bot
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

If your Codex account is entitled to Codex Spark, Hanzo Bot also supports:

- `openai-codex/gpt-5.3-codex-spark`

Hanzo Bot treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

Hanzo Bot also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
Hanzo Bot uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Hanzo Bot also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Hanzo Bot enables it by default for
...
Hanzo Bot, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
Hanzo Bot exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, Hanzo Bot applies a low-latency OpenAI profile:
