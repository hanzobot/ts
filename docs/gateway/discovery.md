...
HanzoBot has two distinct problems that look similar on the surface:
...
- Service types:
  - `_hanzo-bot-gw._tcp` (gateway transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
  ...
  - `cliPath=<path>` (optional; absolute path to a runnable `hanzo-bot` entrypoint or binary)
