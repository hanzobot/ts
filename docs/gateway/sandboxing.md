---
summary: "How HanzoBot sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
HanzoBot can run **tools inside Docker containers** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`hanzo-bot-sandbox-browser`) instead of the global `bridge` network.
  ...
  - noVNC observer access is password-protected by default; HanzoBot emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
HanzoBot mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- HanzoBot blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
...
Default image: `hanzo-bot-sandbox:bookworm-slim`
...
`hanzo-bot-sandbox-common:bookworm-slim`.
