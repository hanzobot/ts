---
summary: "How Hanzo Bot sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
Hanzo Bot can run **tools inside sandbox backends** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - noVNC observer access is password-protected by default; Hanzo Bot emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
Use `backend: "ssh"` when you want Hanzo Bot to sandbox `exec`, file tools, and media reads on
...
- Hanzo Bot creates a per-scope remote root under `sandbox.ssh.workspaceRoot`.
- On first use after create or recreate, Hanzo Bot seeds that remote workspace from the local workspace once.
...
- Hanzo Bot does not sync remote changes back to the local workspace automatically.

Authentication material:

- `identityFile`, `certificateFile`, `knownHostsFile`: use existing local files and pass them through OpenSSH config.
- `identityData`, `certificateData`, `knownHostsData`: use inline strings or SecretRefs. Hanzo Bot resolves them through the normal secrets runtime snapshot, writes them to temp files with `0600`, and deletes them when the SSH session ends.
...
- Host-local edits made outside Hanzo Bot after the seed step are not visible remotely until you recreate the sandbox.
- `hanzo-bot sandbox recreate` deletes the per-scope remote root and seeds again from local on next use.
- Browser sandboxing is not supported on the SSH backend.
- `sandbox.docker.*` settings do not apply to the SSH backend.

### OpenShell backend

Use `backend: "openshell"` when you want Hanzo Bot to sandbox tools in an
...
- `mirror` (default): local workspace stays canonical. Hanzo Bot syncs local files into OpenShell before exec and syncs the remote workspace back after exec.
- `remote`: OpenShell workspace is canonical after the sandbox is created. Hanzo Bot seeds the remote workspace once from the local workspace, then file tools and exec run directly against the remote sandbox without syncing changes back.

Remote transport details:

- Hanzo Bot asks OpenShell for sandbox-specific SSH config via `openshell sandbox ssh-config <name>`.
...
- Before `exec`, Hanzo Bot syncs the local workspace into the OpenShell sandbox.
- After `exec`, Hanzo Bot syncs the remote workspace back to the local workspace.
- File tools still operate through the sandbox bridge, but the local workspace remains the source of truth between turns.

Use this when:

- you edit files locally outside Hanzo Bot and want those changes to show up in the sandbox automatically
...
- When the sandbox is first created, Hanzo Bot seeds the remote workspace from the local workspace once.
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate directly against the remote OpenShell workspace.
- Hanzo Bot does **not** sync remote changes back into the local workspace after exec.
...
- If you edit files on the host outside Hanzo Bot after the seed step, the remote sandbox will **not** see those changes automatically.
...
- `hanzo-bot sandbox list` shows OpenShell runtimes as well as Docker runtimes
- `hanzo-bot sandbox recreate` deletes the current runtime and lets Hanzo Bot recreate it on next use
...
- `"none"` (default): tools see a sandbox workspace under `~/.hanzo/bot/sandboxes`.
...
Hanzo Bot mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- Hanzo Bot blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
...
- Use `hanzo-bot sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
