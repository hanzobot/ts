...
Hanzo Bot supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `BOT_GATEWAY_TOKEN` (or `BOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Hanzo Bot runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Hanzo Bot validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Hanzo Bot QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
...
- Hanzo Bot resolves these refs during sandbox activation, not lazily during each SSH call.
...
When reload-time activation fails after a healthy state, Hanzo Bot enters degraded secrets state.
...
- Read-only command paths (for example `hanzo-bot status`, `hanzo-bot status --all`, `hanzo-bot channels status`, `hanzo-bot channels resolve`, `hanzo-bot security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Use `hanzo-bot secrets audit --allow-exec` to execute exec providers during audit.
...
Hanzo Bot intentionally does not write rollback backups containing historical plaintext secret values.
