...
HanzoBot’s Gateway exposes a simple HTTP endpoint for invoking a single tool directly. It is always enabled, but gated by Gateway auth and tool policy.
...
- `x-hanzo-bot-message-channel: <channel>` (example: `slack`, `telegram`)
- `x-hanzo-bot-account-id: <accountId>` (when multiple accounts exist)
