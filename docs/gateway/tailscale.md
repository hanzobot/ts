...
HanzoBot can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. HanzoBot requires a shared password.
...
(`tailscale-user-login`) without supplying a token/password. HanzoBot verifies
...
- Set `gateway.tailscale.resetOnExit` if you want HanzoBot to undo `tailscale serve`
