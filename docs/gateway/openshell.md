---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Hanzo Bot agents"
read_when:
...
OpenShell is a managed sandbox backend for Hanzo Bot. Instead of running Docker
containers locally, Hanzo Bot delegates sandbox lifecycle to the `openshell` CLI,
...
- Hanzo Bot Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Hanzo Bot creates an OpenShell
...
- You edit files locally outside Hanzo Bot and want those changes visible in the
...
- When the sandbox is first created, Hanzo Bot seeds the remote workspace from
...
- Hanzo Bot does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Hanzo Bot after the initial seed,
the remote sandbox does **not** see those changes. Use
`hanzo-bot sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"@hanzo/bot"`  | Sandbox source for first-time create                  |
...
1. Hanzo Bot calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Hanzo Bot calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `hanzo-bot sandbox` commands
