...
read_when:
  - Running HanzoBot behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of HanzoBot
...
- You run HanzoBot behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. HanzoBot checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. HanzoBot extracts the user identity from the configured header
...
- HanzoBot can stay on loopback HTTP behind the proxy.
...
If HanzoBot itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://hanzo-bot.example.com
    to: http://hanzo-bot-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
hanzo-bot.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy hanzo-bot:18789 {
...
location / {
    ...
    proxy_pass http://hanzo-bot:18789;
...
`hanzo-bot security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update HanzoBot config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `hanzo-bot security audit` and review findings
