...
read_when:
  - Running Hanzo Bot behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Hanzo Bot
...
- You run Hanzo Bot behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Hanzo Bot checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Hanzo Bot extracts the user identity from the configured header
...
- Hanzo Bot can stay on loopback HTTP behind the proxy.
...
If Hanzo Bot itself serves HTTPS directly (no TLS-terminating proxy), set:
...
3. Update Hanzo Bot config with trusted-proxy auth
