...
> HanzoBot is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
HanzoBot security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
hanzo-bot security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

HanzoBot is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
- If multiple users want HanzoBot, use one VPS/host per user.
...
HanzoBot is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
4. Claim is not listed in [Out of Scope](https://github.com/hanzo-bot/hanzo-bot/blob/main/SECURITY.md#out-of-scope).
...
If you run `--deep`, HanzoBot also attempts a best-effort live Gateway probe.
...
`hanzo-bot security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in HanzoBot config
schema:
...
- HanzoBot gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from HanzoBot responses.
...
HanzoBot can refresh the skills list mid-session:
...
By default, HanzoBot routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
HanzoBot includes explicit bypass flags that disable external-content safety wrapping:
...
If you run HanzoBot with Docker on a VPS, remember that published container ports
...
The Gateway broadcasts its presence via mDNS (`_hanzo-bot-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `BOT_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), HanzoBot
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. HanzoBot verifies the identity by resolving the
...
- HanzoBot will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@hanzo-bot", "@mybot"] }
      }
    ]
  }
}
...
- Prefer a dedicated profile for the agent (the default `hanzo-bot` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only HanzoBot clients can connect.
- Disable browser proxy routing when you don’t need it (`gateway.nodes.browser.mode="off"`).
- Chrome extension relay mode is **not** “safer”; it can take over your existing Chrome tabs. Assume it can act as you in whatever that tab/profile can reach.

### Browser SSRF policy (trusted-network default)

HanzoBot’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default HanzoBot limits these tools
...
- Timestamp, gateway host OS + HanzoBot version
...
Found a vulnerability in HanzoBot? Please report responsibly:

1. Email: [security@hanzo-bot.ai](mailto:security@hanzo-bot.ai)
