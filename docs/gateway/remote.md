...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “Hanzo Bot runs”).
...
- `hanzo-bot health` and `hanzo-bot status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `hanzo-bot gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.
...
- URL override safety:
  ...
  - Env URL overrides (`BOT_GATEWAY_URL`) may use env credentials only (`BOT_GATEWAY_TOKEN` / `BOT_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `BOT_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `BOT_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
...
- Legacy `BOT_GATEWAY_*` env vars are only used by compatibility call paths; probe/status/auth resolution uses `BOT_GATEWAY_*` only.
...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  set `BOT_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
