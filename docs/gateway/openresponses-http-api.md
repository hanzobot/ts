...
HanzoBot’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
- `model: "hanzo-bot:<agentId>"` (example: `"hanzo-bot:main"`, `"hanzo-bot:beta"`)
- `model: "agent:<agentId>"` (alias)

Or target a specific HanzoBot agent by header:

- `x-hanzo-bot-agent-id: <agentId>` (default: `main`)

Advanced:

- `x-hanzo-bot-session-key: <sessionKey>` to fully control session routing.
