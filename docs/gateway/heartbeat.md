...
- During heartbeat runs, HanzoBot treats `HEARTBEAT_OK` as an ack when it appears
...
If **all three** are false, HanzoBot skips the heartbeat run entirely (no model call).
