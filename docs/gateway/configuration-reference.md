...
description: "Complete field-by-field reference for ~/.hanzoai/bot.json"
summary: "Complete reference for every Hanzo Bot config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.hanzoai/bot.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Hanzo Bot uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `hanzo-bot doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `hanzo-bot doctor` warns when this is missing or invalid.
...
- Hanzo Bot additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `hanzo-bot plugins install @hanzo/bot-mattermost`.
...
- `commands.callbackUrl` must resolve to the Hanzo Bot gateway endpoint and be reachable from the Mattermost server.
...
Hanzo Bot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `hanzo-bot channels add` (or channel onboarding) while still on a single-account top-level channel config, Hanzo Bot moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `hanzo-bot doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@openclaw", "@hanzo/bot"] } }],
  },
}
```
...
Optional repository root shown in the system prompt's Runtime line. If unset, Hanzo Bot auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Hanzo Bot assumes `anthropic` (deprecated).
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before Hanzo Bot aborts it. Default: `900`.
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that Hanzo Bot materializes into temp files at runtime
...
In `remote` mode, host-local edits made outside Hanzo Bot are not synced into the sandbox automatically after the seed step.
...
noVNC observer access uses VNC auth by default and Hanzo Bot emits a short-lived token URL (instead of exposing the password in the shared URL).
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `BOT_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `BOT_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `BOT_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
...
For `type: "acp"` entries, Hanzo Bot resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Hanzo Bot starts a fresh thread session instead of inheriting parent transcript history.
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Hanzo Bot treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
Hanzo Bot uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.hanzo/bot/agents/<agentId>/agent/models.json`.
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Hanzo Bot forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `hanzo-bot onboard --auth-choice opencode-zen` or `hanzo-bot onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `hanzo-bot onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `hanzo-bot onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `hanzo-bot onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `hanzo-bot onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `hanzo-bot onboard --auth-choice minimax-api`.
...
- Discovery accepts native Hanzo Bot plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Hanzo Bot plugin schema when available).
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Hanzo Bot applies those as sanitized agent settings, not as raw Hanzo Bot config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `hanzo-bot plugins update`.
...
- `BOT_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `BOT_APNS_RELAY_BASE_URL` / `BOT_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `BOT_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
- Hostname defaults to `openclaw`. Override with `BOT_MDNS_HOSTNAME`.
...
- Default log file: `/tmp/hanzoai/bot-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Hanzo Bot.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `BOT_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `hanzo-bot doctor --fix` can strip unknown keys).
