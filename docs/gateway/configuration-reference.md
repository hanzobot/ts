...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["hanzo-bot-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-hanzo-bot",
          requireMention: false,
...
- HanzoBot additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
- `commands.callbackUrl` must resolve to the HanzoBot gateway endpoint and be reachable from the Mattermost server.
...
HanzoBot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `hanzo-bot channels add` (or channel onboarding) while still on a single-account top-level channel config, HanzoBot moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@hanzo-bot", "hanzo-bot"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@hanzo-bot"] },
      },
    ],
  },
}
...
Optional repository root shown in the system prompt's Runtime line. If unset, HanzoBot auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/hanzo-bot" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, HanzoBot assumes `anthropic` (deprecated).
...
- `network` defaults to `hanzo-bot-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        groupChat: { mentionPatterns: ["@hanzo-bot"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
For `type: "acp"` entries, HanzoBot resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, HanzoBot starts a fresh thread session instead of inheriting parent transcript history.
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, HanzoBot treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:hanzo-bot`  | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.hanzo-bot/attachments/<uuid>/` with a `.manifest.json`.
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), HanzoBot forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `hanzo-bot onboard --auth-choice opencode-zen`.
...
```json5
{
  browser: {
    ...
    profiles: {
      hanzo-bot: { cdpPort: 18800, color: "#FF4500" },
...
Auth: `Authorization: Bearer <token>` or `x-hanzo-bot-token: <token>`.
...
- Hostname defaults to `hanzo-bot`. Override with `BOT_MDNS_HOSTNAME`.
...
```json5
{
  secrets: {
    providers: {
      ...
      vault: {
        source: "exec",
        command: "/usr/local/bin/hanzo-bot-vault-resolver",
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/hanzo-bot/hanzo-bot.log",
    consoleLevel: "info",
...
- Default log file: `/tmp/hanzo-bot/hanzo-bot-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one HanzoBot.`).
