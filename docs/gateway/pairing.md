...
hanzo-bot nodes reject <requestId>
hanzo-bot nodes status
hanzo-bot nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.hanzo-bot`):
