...
read_when:
  - Setting up HanzoBot for the first time
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    hanzo-bot onboard       # full setup wizard
...
<Warning>
HanzoBot only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, HanzoBot runs your login shell and imports only the missing keys:
