...
read_when:
  - Setting up Hanzo Bot for the first time
...
Hanzo Bot reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.hanzoai/bot.json`.

If the file is missing, Hanzo Bot uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `hanzo-bot onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    hanzo-bot onboard       # full onboarding flow
    hanzo-bot configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ...
    hanzo-bot config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.hanzoai/bot.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Hanzo Bot only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`hanzo-bot doctor`, `hanzo-bot logs`, `hanzo-bot health`, `hanzo-bot status`)
- Run `hanzo-bot doctor` to see exact issues
- Run `hanzo-bot doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
        ...
        list: [
          {
            id: "main",
            groupChat: {
              mentionPatterns: ["@openclaw", "@hanzo/bot"],
            },
          },
        ],
      },
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    ...
    ```bash
    hanzo-bot config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `BOT_APNS_RELAY_BASE_URL` and `BOT_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `BOT_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.hanzoai/bot.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `hanzo-bot config set` for single keys.
    </Warning>
...
Hanzo Bot reads env vars from the parent process plus:
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Hanzo Bot runs your login shell and imports only the missing keys:
