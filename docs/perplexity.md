...
Hanzo Bot supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Hanzo Bot also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `hanzo-bot configure --section web`. It stores the key in
`~/.hanzoai/bot.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.hanzo/bot/.env` (or your service environment). See [Env vars](/help/faq#how-does-openclaw-load-environment-variables).
