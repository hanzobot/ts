...
read_when:
  ...
  - Deciding: standalone CLI vs deep HanzoBot integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.hanzo-bot/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Recommendation: **deep integration in HanzoBot**, but keep a separable core library.

### Why integrate into HanzoBot?

- HanzoBot already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `hanzo-bot memory recall "…" --k 25 --since 30d`
  - `hanzo-bot memory reflect --since 7d`
