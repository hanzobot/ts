...
This plan defines how HanzoBot should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- HanzoBot core owns ACP control plane concerns
...
HanzoBot should not reimplement ACP transport internals in core.
HanzoBot should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in HanzoBot, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under HanzoBot state dir
...
- ACP event stream is normalized to HanzoBot reply chunks
...
For the first production backend (`extensions/acpx`), HanzoBot and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses HanzoBot `sessionKey`)
...
- HanzoBot consumes ndjson events from `acpx --format json --json-strict`
