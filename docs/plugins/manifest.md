...
read_when:
  - You are building an Hanzo Bot plugin
...
This page is for the **native Hanzo Bot plugin manifest** only.
...
Hanzo Bot auto-detects those bundle layouts too, but they are not validated
against the `openclaw.plugin.json` schema described here.

For compatible bundles, Hanzo Bot currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches Hanzo Bot runtime expectations.

Every native Hanzo Bot plugin **must** ship a `openclaw.plugin.json` file in the
**plugin root**. Hanzo Bot uses this manifest to validate configuration
...
- `providerAuthEnvVars` (object): auth env vars keyed by provider id. Use this
  when Hanzo Bot should resolve provider credentials from env without loading
  plugin runtime first.
- `providerAuthChoices` (array): cheap onboarding/auth-choice metadata keyed by
  provider + auth method. Use this when Hanzo Bot should show a provider in
...
- The manifest is **required for native Hanzo Bot plugins**, including local filesystem loads.
