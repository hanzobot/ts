...
read_when:
  - You want Zalo Personal (unofficial) support in HanzoBot
...
Zalo Personal support for HanzoBot via a plugin, using native `zca-js` to automate a normal Zalo user account.
...
```bash
hanzo-bot plugins install @hanzo/bot-zalouser
```
...
```bash
hanzo-bot plugins install ./extensions/zalouser
...
hanzo-bot message send --channel zalouser --target <threadId> --message "Hello from HanzoBot"
