...
read_when:
  - Building or debugging native Hanzo Bot plugins
...
This page covers the internal architecture of the Hanzo Bot plugin system. For
user-facing setup, discovery, and configuration, see [Plugins](/tools/plugin).

## Public capability model

Capabilities are the public **native plugin** model inside Hanzo Bot. Every
native Hanzo Bot plugin registers against one or more capability types:
...
Hanzo Bot classifies every loaded plugin into a shape based on its actual
...
Use `hanzo-bot plugins inspect <id>` to see a plugin's shape and capability
...
When you run `hanzo-bot doctor` or `hanzo-bot plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `hanzo-bot status --all` and `hanzo-bot plugins doctor`.

## Architecture overview

Hanzo Bot's plugin system has four layers:

1. **Manifest + discovery**
   Hanzo Bot finds candidate plugins from configured paths, workspace roots,
...
3. **Runtime loading**
   Native Hanzo Bot plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Hanzo Bot reads the registry to expose tools, channels, provider
...
That split lets Hanzo Bot validate config, explain missing/disabled plugins, and
...
normal chat actions. Hanzo Bot keeps one shared `message` tool in core, and
...
Hanzo Bot treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Hanzo Bot-facing
  surfaces
...
So if Hanzo Bot adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Hanzo Bot has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { Hanzo BotPluginDefinition } from "openclaw/plugin-sdk";
import {
...
const plugin: Hanzo BotPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Hanzo Bot already treats image/audio/video understanding as one shared
...
If Hanzo Bot adds a new domain later, such as video generation, use the same
...
`Hanzo BotPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Hanzo Bot can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Hanzo Bot knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `Hanzo BotPluginApi` or
  `api.runtime`
...
Native Hanzo Bot plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the Hanzo Bot process

Compatible bundles are safer by default because Hanzo Bot currently treats them
...
Hanzo Bot exports capabilities, not implementation convenience.
...
At startup, Hanzo Bot does roughly this:
...
The manifest is the control-plane source of truth. Hanzo Bot uses it to:
...
Hanzo Bot keeps short in-process caches for:
...
Hanzo Bot still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Hanzo Bot calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | Hanzo Bot tries the normal registry/catalog path first                                    | _(not a plugin hook)_                                                                |
...
that still runs on Hanzo Bot's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Hanzo Bot's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Hanzo Bot adds those
...
- If Hanzo Bot adds a new capability such as video generation later, define the
...
- Hanzo Bot only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Hanzo Bot plugin runtime helpers",
    count: 5,
  },
});
```
...
- Bundled extension internals remain private. External plugins should use only
  `openclaw/plugin-sdk/*` subpaths. Hanzo Bot core/test code may use the repo
...
`catalog.run(...)` returns the same shape Hanzo Bot writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Hanzo Bot's
...
- if both `catalog` and `discovery` are registered, Hanzo Bot uses `catalog`
...
When Hanzo Bot needs setup surfaces for a disabled channel plugin, or
...
this flag. Keep the plugin on the default behavior and let Hanzo Bot load the
...
2. add typed plugin registration/runtime surfaces
   Extend `Hanzo BotPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Hanzo Bot stays opinionated without becoming hardcoded to one
