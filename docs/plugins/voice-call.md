...
read_when:
  - You want to place an outbound voice call from HanzoBot
...
Voice calls for HanzoBot via a plugin. Supports outbound notifications and
...
- Use `hanzo-bot voicecall ...` or the `voice_call` tool
...
```bash
hanzo-bot plugins install ./extensions/voice-call
...
```bash
hanzo-bot voicecall call --to "+15555550123" --message "Hello from HanzoBot"
