...
HanzoBot supports **camera capture** for agent workflows:
...
```bash
hanzo-bot nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
hanzo-bot nodes camera snap --node <id> --facing front
hanzo-bot nodes camera clip --node <id> --duration 3000
hanzo-bot nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`hanzo-bot.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `hanzo-bot` CLI to invoke camera commands on the macOS node.

Examples:

```bash
hanzo-bot nodes camera list --node <id>            # list camera ids
hanzo-bot nodes camera snap --node <id>            # prints MEDIA:<path>
hanzo-bot nodes camera snap --node <id> --max-width 1280
hanzo-bot nodes camera snap --node <id> --delay-ms 2000
hanzo-bot nodes camera snap --node <id> --device-id <id>
hanzo-bot nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
hanzo-bot nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
hanzo-bot nodes camera clip --node <id> --device-id <id>
hanzo-bot nodes camera clip --node <id> --no-audio
```
...
```bash
hanzo-bot nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
