---
summary: "CLI reference for `hanzo-bot secrets` (reload, audit, configure, apply)"
read_when:
...
Use `hanzo-bot secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
hanzo-bot secrets configure
hanzo-bot secrets configure --plan-out /tmp/hanzo-bot-secrets-plan.json
hanzo-bot secrets configure --apply --yes
hanzo-bot secrets configure --providers-only
hanzo-bot secrets configure --skip-provider-setup
hanzo-bot secrets configure --agent ops
hanzo-bot secrets configure --json
```
...
- On Windows, if ACL verification is unavailable for a provider path, HanzoBot fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
hanzo-bot secrets apply --from /tmp/hanzo-bot-secrets-plan.json
hanzo-bot secrets apply --from /tmp/hanzo-bot-secrets-plan.json --dry-run
hanzo-bot secrets apply --from /tmp/hanzo-bot-secrets-plan.json --json
```
