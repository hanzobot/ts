...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Hanzo Bot Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`hanzo-bot acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Hanzo Bot gateway or agent instead.                                                                                                                                     |
...
openclaw acp --url wss://gateway-host:18789 --token-file ~/.hanzo/bot/gateway.token
...
openclaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.hanzo/bot/gateway.token

# Override the server command (default: openclaw)
openclaw acp client --server "node" --server-args hanzo-bot.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive an Hanzo Bot Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `hanzo-bot acp` over stdio.
...
Hanzo Bot bot over ACP, use `acpx` with its built-in `openclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx openclaw` at `hanzo-bot acp`.
3. Target the Hanzo Bot session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Hanzo Bot ACP session
acpx hanzo-bot exec "Summarize the active Hanzo Bot session state."

# Persistent named session for follow-up turns
acpx hanzo-bot sessions ensure --name codex-bridge
acpx hanzo-bot -s codex-bridge --cwd /path/to/repo \
  "Ask my Hanzo Bot work agent for recent context relevant to this repo."
```
...
```json
{
  "agents": {
    "@hanzo/bot": {
      "command": "env BOT_HIDE_BANNER=1 BOT_SUPPRESS_NOTES=1 hanzo-bot acp --url ws://127.0.0.1:18789 --token-file ~/.hanzo/bot/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Hanzo Bot checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env BOT_HIDE_BANNER=1 BOT_SUPPRESS_NOTES=1 node hanzo-bot.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Hanzo Bot agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Hanzo Bot ACP” to start a thread.
...
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`BOT_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- `hanzo-bot acp client` sets `BOT_SHELL=acp-client` on the spawned bridge process.
