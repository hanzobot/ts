---
summary: "Hanzo Bot CLI reference for `openclaw` commands, subcommands, and options"
read_when:
...
Note: plugins can add additional top-level commands (for example `hanzo-bot voicecall`).

## Security

- `hanzo-bot security audit` — audit config + local state for common security foot-guns.
- `hanzo-bot security audit --deep` — best-effort live Gateway probe.
- `hanzo-bot security audit --fix` — tighten safe defaults and chmod state/config.

## Secrets

- `hanzo-bot secrets reload` — re-resolve refs and atomically swap the runtime snapshot.
- `hanzo-bot secrets audit` — scan for plaintext residues, unresolved refs, and precedence drift (`--allow-exec` to execute exec providers during audit).
- `hanzo-bot secrets configure` — interactive helper for provider setup + SecretRef mapping + preflight/apply (`--allow-exec` to execute exec providers during preflight and exec-containing apply flows).
- `hanzo-bot secrets apply --from <plan.json>` — apply a previously generated plan (`--dry-run` supported; use `--allow-exec` to permit exec providers in dry-run and exec-containing write plans).

## Plugins

Manage extensions and their config:

- `hanzo-bot plugins list` — discover plugins (use `--json` for machine output).
- `hanzo-bot plugins inspect <id>` — show details for a plugin (`info` is an alias).
- `hanzo-bot plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`).
- `hanzo-bot plugins marketplace list <marketplace>` — list marketplace entries before install.
- `hanzo-bot plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `hanzo-bot plugins doctor` — report plugin load errors.
...
- `hanzo-bot memory status` — show index stats.
- `hanzo-bot memory index` — reindex memory files.
- `hanzo-bot memory search "<query>"` (or `--query "<query>"`) — semantic search over memory.
...
Non-interactive config helpers (get/set/unset/file/validate). Running `hanzo-bot config` with no
...
- `channels status`: check gateway reachability and channel health (`--probe` runs extra checks; use `hanzo-bot health` or `hanzo-bot status --deep` for gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `hanzo-bot doctor`).
- `channels logs`: show recent channel logs from the gateway log file.
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, Hanzo Bot moves account-scoped values into `channels.<channel>.accounts.default` before writing the new account.
...
- `hanzo-bot message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, Hanzo Bot may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
Hanzo Bot can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `hanzo-bot status --usage` (prints full provider breakdown)
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named Hanzo Bot services are treated as first-class and aren't flagged as "extra".
...
`hanzo-bot models` is an alias for `models status`.
...
- `node` resolves gateway auth from env/config (no `--token`/`--password` flags): `BOT_GATEWAY_TOKEN` / `BOT_GATEWAY_PASSWORD`, then `gateway.auth.*`. In local mode, node host intentionally ignores `gateway.remote.*`; in `gateway.mode=remote`, `gateway.remote.*` participates per remote precedence rules.
- Legacy `BOT_GATEWAY_*` env vars are intentionally ignored for node-host auth resolution.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`hanzo-bot browser`](/cli/browser) and the [Browser tool](/tools/browser).
