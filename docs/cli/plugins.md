...
Bundled plugins ship with HanzoBot but start disabled. Use `plugins enable` to
activate them.

All plugins must ship a `hanzo-bot.plugin.json` file with an inline JSON Schema
...
If a bare install spec matches a bundled plugin id (for example `diffs`), HanzoBot
