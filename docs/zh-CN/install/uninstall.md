...
summary: 完全卸载 HanzoBot（CLI、服务、状态、工作区）
title: 卸载
x-i18n:
...
如果 Gateway 网关服务持续运行但 `hanzo-bot` 缺失，请使用此方法。

### macOS（launchd）

默认标签是 `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.hanzo-bot.*` 可能仍然存在）：
...
如果你使用了配置文件，请将标签和 plist 名称替换为 `bot.molt.<profile>`。如果存在任何旧版 `com.hanzo-bot.*` plist，请将其移除。

### Linux（systemd 用户单元）

默认单元名称是 `hanzo-bot-gateway.service`（或 `hanzo-bot-gateway-<profile>.service`）：
...
默认任务名称是 `HanzoBot Gateway`（或 `HanzoBot Gateway (<profile>)`）。
任务脚本位于你的状态目录下。
...
如果你使用了 `https://hanzo-bot.ai/install.sh` 或 `install.ps1`，CLI 是通过 `npm install -g hanzo-bot@latest` 安装的。
使用 `npm rm -g hanzo-bot` 移除（或 `pnpm remove -g` / `bun remove -g`，如果你是用那种方式安装的）。

### 源码检出（git clone）

如果你从仓库检出运行（`git clone` + `hanzo-bot ...` / `bun run hanzo-bot ...`）：
