...
summary: 完全卸载 Hanzo Bot（CLI、服务、状态、工作区）
title: 卸载
x-i18n:
...
默认任务名称是 `Hanzo Bot Gateway`（或 `Hanzo Bot Gateway (<profile>)`）。
任务脚本位于你的状态目录下。
...
如果你使用了 `https://hanzo.bot/install.sh` 或 `install.ps1`，CLI 是通过 `npm install -g @hanzo/bot@latest` 安装的。
