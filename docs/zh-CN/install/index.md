...
read_when:
  ...
  - 你想部署到云平台
  - 你需要更新、迁移或卸载
summary: 安装 Hanzo Bot —— 安装脚本、npm/pnpm、从源码、Docker 等
title: 安装
x-i18n:
...
<Note>
在 Windows 上，我们强烈建议你在 [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) 下运行 Hanzo Bot。
</Note>

## 安装方法

<Tip>
**安装脚本** 是安装 Hanzo Bot 的推荐方式。它会一步完成 Node 检测、安装和新手引导。
</Tip>

<Warning>
对于 VPS/云主机，尽量避免使用第三方“一键式”市场镜像。优先选择干净的基础 OS 镜像（例如 Ubuntu LTS），然后使用安装脚本自行安装 Hanzo Bot。
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="npm / pnpm" icon="package">
    如果你已经自行管理 Node，我们推荐使用 Node 24。出于兼容性考虑，Hanzo Bot 仍支持 Node 22 LTS，目前为 `22.16+`：
    ...
    ```bash
    npm install -g github:hanzoai/bot#main
    ```

    ```bash
    pnpm add -g github:hanzoai/bot#main
    ```
  ...
  <Accordion title="从源码" icon="github">
    ...
    <Steps>
      <Step title="克隆并构建">
        克隆 [Hanzo Bot 仓库](https://github.com/hanzoai/bot) 并构建：
      ...
      <Step title="链接 CLI">
        ...
        或者，你也可以跳过链接，直接在仓库内通过 `pnpm hanzo-bot ...` 运行命令。
      </Step>
      <Step title="运行新手引导">
        ```bash
        hanzo-bot onboard --install-daemon
...
- `BOT_HOME` 用于基于主目录的内部路径
- `BOT_STATE_DIR` 用于可变状态位置
- `BOT_CONFIG_PATH` 用于配置文件位置
