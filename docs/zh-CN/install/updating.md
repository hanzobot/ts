...
read_when:
  - 更新 Hanzo Bot
  - 更新后出现问题
summary: 安全更新 Hanzo Bot（全局安装或源码），以及回滚策略
title: 更新
x-i18n:
...
Hanzo Bot 发展迅速（尚未到"1.0"）。将更新视为发布基础设施：更新 → 运行检查 → 重启（或使用会重启的 `hanzo-bot update`）→ 验证。
...
- 当你运行打包的 `openclaw` 二进制文件（[`hanzo-bot.mjs`](https://github.com/hanzoai/bot/blob/main/hanzo-bot.mjs)）或使用 Node 运行 `dist/` 时，`pnpm build` 很重要。
...
- 检测并将旧版 Gateway 网关服务（launchd/systemd；旧版 schtasks）迁移到当前 Hanzo Bot 服务。
