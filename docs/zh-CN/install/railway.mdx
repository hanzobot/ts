...
通过一键模板在 Railway 上部署 HanzoBot，并在浏览器中完成设置。
...
- `https://<your-railway-domain>/hanzo-bot` — 控制面板 UI
...
- `BOT_STATE_DIR=/data/.hanzo-bot`（推荐）
- `BOT_WORKSPACE_DIR=/data/workspace`（推荐）
- `BOT_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 HanzoBot 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
