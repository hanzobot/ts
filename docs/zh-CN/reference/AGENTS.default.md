...
read_when:
  - 启动新的 HanzoBot 智能体会话
  - 启用或审计默认 Skills
summary: 个人助手设置的默认 HanzoBot 智能体指令和 Skills 列表
x-i18n:
...
- macOS 应用管理权限（屏幕录制、通知、麦克风）并通过其内置二进制文件暴露 `hanzo-bot` CLI。
...
- 脚本编写优先使用 `hanzo-bot` CLI；mac 应用处理权限。
...
- 对于浏览器驱动的验证，使用带有 HanzoBot 管理的 Chrome 配置文件的 `hanzo-bot browser`（tabs/status/screenshot）。
