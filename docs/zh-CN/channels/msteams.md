...
- 通过 Teams 私信、群聊或频道与 HanzoBot 交流。
...
- 启动时，HanzoBot 将团队/频道和用户允许列表名称解析为 ID（当 Graph 权限允许时）
  并记录映射；未解析的条目保持原样。
...
在配置 HanzoBot 之前，你需要创建一个 Azure Bot 资源。
...
2. 填写**基本信息**选项卡：
   ...
   | 字段               | 值                                                   |
   | ------------------ | ---------------------------------------------------- |
   | **Bot handle**     | 你的机器人名称，例如 `hanzo-bot-msteams`（必须唯一） |
...
1. **安装 Microsoft Teams 插件**
   - 从 npm：`hanzo-bot plugins install @hanzo/bot-msteams`
   - 从本地检出：`hanzo-bot plugins install ./extensions/msteams`
...
HanzoBot 通过快速返回并主动发送回复来处理这个问题，但非常慢的响应仍可能导致问题。
...
默认情况下，HanzoBot 仅从 Microsoft/Teams 主机名下载媒体。使用 `channels.msteams.mediaAllowHosts` 覆盖（使用 `["*"]` 允许任何主机）。
...
上传的文件存储在配置的 SharePoint 站点默认文档库中的 `/HanzoBotShared/` 文件夹中。
