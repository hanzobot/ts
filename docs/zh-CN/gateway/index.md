...
- 替代旧版 `gateway` 命令。CLI 入口点：`hanzo-bot gateway`。
...
# 如果端口被占用，终止监听器然后启动：
hanzo-bot gateway --force
# 开发循环（TS 更改时自动重载）：
pnpm gateway:watch
```

- 配置热重载监视 `~/.hanzo-bot/hanzo-bot.json`（或 `BOT_CONFIG_PATH`）。
...
- macOS：`bot.molt.<profile>`（旧版 `com.hanzo-bot.*` 可能仍然存在）
- Linux：`hanzo-bot-gateway-<profile>.service`
- Windows：`HanzoBot Gateway (<profile>)`
...
```bash
hanzo-bot --dev setup
hanzo-bot --dev gateway --allow-unconfigured
# 然后定位到 dev 实例：
hanzo-bot --dev status
hanzo-bot --dev health
```
...
- `health` — 完整健康快照（与 `hanzo-bot health --json` 形状相同）。
- `status` — 简短摘要。
...
- LaunchAgents 是按用户的，需要已登录的会话；对于无头设置，使用自定义 LaunchDaemon（未随附）。
  - `hanzo-bot gateway install` 写入 `~/Library/LaunchAgents/bot.molt.gateway.plist`
    （或 `bot.molt.<profile>.plist`；旧版 `com.hanzo-bot.*` 会被清理）。
  - `hanzo-bot doctor` 审计 LaunchAgent 配置，可以将其更新为当前默认值。
...
- 如果检测到其他类似 Gateway 网关的服务，CLI 会发出警告，除非它们是 HanzoBot 配置文件服务。
  我们仍然建议大多数设置**每台机器一个 Gateway 网关**；使用隔离的配置文件/端口进行冗余或救援机器人。参见[多个 Gateway 网关](/gateway/multiple-gateways)。
  - 清理：`hanzo-bot gateway uninstall`（当前服务）和 `hanzo-bot doctor`（旧版迁移）。
- `gateway install` 在已安装时是无操作的；使用 `hanzo-bot gateway install --force` 重新安装（配置文件/env/路径更改）。

捆绑的 mac 应用：

- HanzoBot.app 可以捆绑基于 Node 的 Gateway 网关中继并安装标记为
  `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.hanzo-bot.*` 标签仍能干净卸载）的按用户 LaunchAgent。
- 要干净地停止它，使用 `hanzo-bot gateway stop`（或 `launchctl bootout gui/$UID/bot.molt.gateway`）。
- 要重启，使用 `hanzo-bot gateway restart`（或 `launchctl kickstart -k gui/$UID/bot.molt.gateway`）。
  - `launchctl` 仅在 LaunchAgent 已安装时有效；否则先使用 `hanzo-bot gateway install`。
  - 运行命名配置文件时，将标签替换为 `bot.molt.<profile>`。

## 监管（systemd 用户单元）

HanzoBot 在 Linux/WSL2 上默认安装 **systemd 用户服务**。我们
建议单用户机器使用用户服务（更简单的 env，按用户配置）。
对于多用户或常驻服务器使用**系统服务**（无需 lingering，
共享监管）。

`hanzo-bot gateway install` 写入用户单元。`hanzo-bot doctor` 审计
单元并可以将其更新以匹配当前推荐的默认值。

创建 `~/.config/systemd/user/hanzo-bot-gateway[-<profile>].service`：

```
[Unit]
Description=HanzoBot Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/hanzo-bot gateway --port 18789
Restart=always
RestartSec=5
...
```
systemctl --user enable --now hanzo-bot-gateway[-<profile>].service
```

**替代方案（系统服务）** - 对于常驻或多用户服务器，你可以
安装 systemd **系统**单元而不是用户单元（无需 lingering）。
创建 `/etc/systemd/system/hanzo-bot-gateway[-<profile>].service`（复制上面的单元，
切换 `WantedBy=multi-user.target`，设置 `User=` + `WorkingDirectory=`），然后：

```
sudo systemctl daemon-reload
sudo systemctl enable --now hanzo-bot-gateway[-<profile>].service
```
...
- `hanzo-bot gateway health|status` — 通过 Gateway 网关 WS 请求 health/status。
- `hanzo-bot message send --target <num> --message "hi" [--media ...]` — 通过 Gateway 网关发送（对 WhatsApp 是幂等的）。
- `hanzo-bot agent --message "hi" --to <num>` — 运行智能体轮次（默认等待最终结果）。
- `hanzo-bot gateway call <method> --params '{"k":"v"}'` — 用于调试的原始方法调用器。
- `hanzo-bot gateway stop|restart` — 停止/重启受监管的 Gateway 网关服务（launchd/systemd）。
- Gateway 网关辅助子命令假设 `--url` 上有运行中的 Gateway 网关；它们不再自动生成一个。

## 迁移指南

- 淘汰 `hanzo-bot gateway` 和旧版 TCP 控制端口的使用。
