...
- 配置热重载监视 `~/.hanzoai/bot.json`（或 `BOT_CONFIG_PATH`）。
...
- 默认在 `canvasHost.port`（默认 `18793`）上启动 Canvas 文件服务器，从 `~/.hanzo/bot/workspace/canvas` 提供 `http://<gateway-host>:18793/__openclaw__/canvas/`。使用 `canvasHost.enabled=false` 或 `BOT_SKIP_CANVAS_HOST=1` 禁用。
...
- Windows：`Hanzo Bot Gateway (<profile>)`
...
```bash
BOT_CONFIG_PATH=~/.hanzo/bot/a.json BOT_STATE_DIR=~/.openclaw-a hanzo-bot gateway --port 19001
BOT_CONFIG_PATH=~/.hanzo/bot/b.json BOT_STATE_DIR=~/.openclaw-b hanzo-bot gateway --port 19002
```
...
- 如果检测到其他类似 Gateway 网关的服务，CLI 会发出警告，除非它们是 Hanzo Bot 配置文件服务。
...
- Hanzo Bot.app 可以捆绑基于 Node 的 Gateway 网关中继并安装标记为
...
Hanzo Bot 在 Linux/WSL2 上默认安装 **systemd 用户服务**。我们
...
```
[Unit]
Description=Hanzo Bot Gateway (profile: <profile>, v<version>)
