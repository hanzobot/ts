...
read_when:
  - 调查运行时问题或故障
summary: Hanzo Bot 常见故障的快速故障排除指南
title: 故障排除
x-i18n:
...
- 文件日志（始终）：`/tmp/hanzoai/bot-YYYY-MM-DD.log`（或你配置的 `logging.file`）
...
Hanzo Bot 有意拒绝**较旧/不安全的模型**（尤其是那些更容易受到提示词注入攻击的模型）。如果你看到此错误，该模型名称已不再支持。
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${BOT_CONFIG_PATH:-$HOME/.hanzoai/bot.json}"
```
...
# 或者如果你想快速过滤：
tail -f "$(ls -t /tmp/hanzoai/bot-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/hanzoai/bot-*.log | head -1)" | tail -20
```

### 高内存使用

Hanzo Bot 在内存中保留对话历史。
...
当配置包含未知键、格式错误的值或无效类型时，Hanzo Bot 现在拒绝启动。
这是为了安全而故意设计的。
...
端点接受 JSON Schema 的严格子集。Hanzo Bot 在当前 `main` 中清理/规范化工具
...
如果重置不起作用，在 [`scripts/package-mac-app.sh`](https://github.com/hanzoai/bot/blob/main/scripts/package-mac-app.sh) 中更改 `BUNDLE_ID`（例如，添加 `.test` 后缀）并重新构建。这会强制 macOS 将其视为新应用。
...
```bash
# 在配置中打开跟踪日志：
#   ${BOT_CONFIG_PATH:-$HOME/.hanzoai/bot.json} -> { logging: { level: "trace" } }
#
...
| 日志                             | 位置                                                                                                                                                                                                                                                                                                                      |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway 网关文件日志（结构化）   | `/tmp/hanzoai/bot-YYYY-MM-DD.log`（或 `logging.file`）                                                                                                                                                                                                                                                              |
| Gateway 网关服务日志（监管程序） | macOS：`$BOT_STATE_DIR/logs/gateway.log` + `gateway.err.log`（默认：`~/.hanzo/bot/logs/...`；配置文件使用 `~/.openclaw-<profile>/logs/...`）<br />Linux：`journalctl --user -u openclaw-gateway[-<profile>].service -n 200 --no-pager`<br />Windows：`schtasks /Query /TN "Hanzo Bot Gateway (<profile>)" /V /FO LIST` |
