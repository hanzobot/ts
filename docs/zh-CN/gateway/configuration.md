...
HanzoBot 只接受完全匹配 schema 的配置。
未知键、类型错误或无效值会导致 Gateway 网关 **拒绝启动**以确保安全。

验证失败时：

- Gateway 网关不会启动。
- 只允许诊断命令（例如：`hanzo-bot doctor`、`hanzo-bot logs`、`hanzo-bot health`、`hanzo-bot status`、`hanzo-bot service`、`hanzo-bot help`）。
- 运行 `hanzo-bot doctor` 查看具体问题。
- 运行 `hanzo-bot doctor --fix`（或 `--yes`）应用迁移/修复。
...
```bash
hanzo-bot gateway call config.get --params '{}' # capture payload.hash
hanzo-bot gateway call config.patch --params '{
...
```json5
{
  agents: {
    ...
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@hanzo-bot", "reisponde"] },
      },
    ],
  },
  channels: {
...
{ "$include": "/etc/hanzo-bot/base.json5" } // 绝对路径
...
HanzoBot 从父进程（shell、launchd/systemd、CI 等）读取环境变量。
...
可选便利功能：如果启用且预期键均未设置，HanzoBot 会运行你的登录 shell 并仅导入缺失的预期键（不会覆盖）。
...
- OAuth 目录（仅旧版导入）：`BOT_OAUTH_DIR`
- 智能体目录（默认智能体根目录覆盖）：`BOT_AGENT_DIR`（推荐）、`PI_CODING_AGENT_DIR`（旧版）

首次使用时，HanzoBot 会将 `oauth.json` 条目导入到 `auth-profiles.json` 中。
...
如果设置了，HanzoBot 会推导默认值（仅在你未明确设置时）：
...
- 默认日志文件：`/tmp/hanzo-bot/hanzo-bot-YYYY-MM-DD.log`
- 如需稳定路径，将 `logging.file` 设为 `/tmp/hanzo-bot/hanzo-bot.log`。
- 控制台输出可通过以下方式单独调整：
...
- 旧版单账号 Baileys 认证目录由 `hanzo-bot doctor` 迁移到 `whatsapp/default`。
    ...
    list: [
      {
        ...
        groupChat: {
          // 仅这些文本模式会触发响应
          mentionPatterns: ["reisponde", "@hanzo-bot"],
        },
      },
    ],
  },
...
HanzoBot 仅在存在 `channels.telegram` 配置段时启动 Telegram。机器人 token 从 `channels.telegram.botToken`（或 `channels.telegram.tokenFile`）解析，`TELEGRAM_BOT_TOKEN` 作为默认账号的回退。
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["hanzo-bot-dm"], // 可选群组私聊白名单
      },
      guilds: {
        "123456789012345678": {
          // 服务器 id（推荐）或 slug
...
HanzoBot 仅在存在 `channels.discord` 配置段时启动 Discord。token 从 `channels.discord.token` 解析，`DISCORD_BOT_TOKEN` 作为默认账号的回退（除非 `channels.discord.enabled` 为 `false`）。在为 cron/CLI 命令指定投递目标时，使用 `user:<id>`（私聊）或 `channel:<id>`（服务器频道）；裸数字 ID 有歧义会被拒绝。
...
HanzoBot 在提供商启用且两个 token 都已设置时启动 Slack（通过配置或 `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`）。在为 cron/CLI 命令指定投递目标时使用 `user:<id>`（私聊）或 `channel:<id>`。
...
请先安装：`hanzo-bot plugins install @hanzo/bot-mattermost`（或从 git checkout 使用 `./extensions/mattermost`）。
...
HanzoBot 在账号已配置（机器人 token + 基础 URL）且已启用时启动 Mattermost。token + 基础 URL 从 `channels.mattermost.botToken` + `channels.mattermost.baseUrl` 或默认账号的 `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` 解析（除非 `channels.mattermost.enabled` 为 `false`）。
...
HanzoBot 会生成 `imsg rpc`（通过 stdio 的 JSON-RPC）。无需守护进程或端口。
...
在系统提示的 Runtime 行中显示的可选仓库根目录。如果未设置，HanzoBot 会从工作区（和当前工作目录）向上查找 `.git` 目录进行检测。路径必须存在才能使用。
...
当文件超过此限制时，HanzoBot 会记录警告并注入带标记的头尾截断内容。
...
设置用户时区用于**系统提示上下文**（不用于消息信封中的时间戳）。如果未设置，HanzoBot 在运行时使用主机时区。
...
如果未设置 `messages.responsePrefix`，默认不应用前缀。WhatsApp 自聊天回复是例外：它们在设置时默认为 `[{identity.name}]`，否则为 `[hanzo-bot]`，以保持同一手机上的对话可读性。
...
WhatsApp 入站前缀通过 `channels.whatsapp.messagePrefix` 配置（已弃用：`messages.messagePrefix`）。默认保持**不变**：当 `channels.whatsapp.allowFrom` 为空时为 `"[hanzo-bot]"`，否则为 `""`（无前缀）。使用 `"[hanzo-bot]"` 时，如果路由的智能体设置了 `identity.name`，HanzoBot 会改用 `[{identity.name}]`。
...
为出站回复启用文字转语音。开启后，HanzoBot 使用 ElevenLabs 或 OpenAI 生成音频并附加到回复中。Telegram 使用 Opus 语音消息；其他渠道发送 MP3 音频。
...
HanzoBot 还内置了一些别名快捷方式。默认值仅在模型已存在于 `agents.defaults.models` 中时才应用：
...
如果省略提供商，HanzoBot 目前假定 `anthropic` 作为临时弃用回退。
...
- `tools.web.search.apiKey`（推荐：通过 `hanzo-bot configure --section web` 设置，或使用 `BRAVE_API_KEY` 环境变量）
...
- `group:nodes`：`nodes`
- `group:hanzo-bot`：所有内置 HanzoBot 工具（不包含提供商插件）
...
OpenCode Zen 是一个具有每模型端点的多模型网关。HanzoBot 使用
...
- 快捷方式：`hanzo-bot onboard --auth-choice opencode-zen`。
...
快捷方式：`hanzo-bot onboard --auth-choice zai-api-key`。
...
- 在环境中设置 `MOONSHOT_API_KEY` 或使用 `hanzo-bot onboard --auth-choice moonshot-api-key`。
...
- 在环境中设置 `KIMI_API_KEY` 或使用 `hanzo-bot onboard --auth-choice kimi-code-api-key`。
...
- 设置 `SYNTHETIC_API_KEY` 或使用 `hanzo-bot onboard --auth-choice synthetic-api-key`。
...
- 设置 `MINIMAX_API_KEY` 环境变量或使用 `hanzo-bot onboard --auth-choice minimax-api`。
...
- `resetByType`：`dm`、`group` 和 `thread` 的每会话覆盖。
  - 如果你只设置了旧版 `session.idleMinutes` 而没有任何 `reset`/`resetByType`，HanzoBot 保持仅空闲模式以向后兼容。
...
### `browser`（HanzoBot 管理的浏览器）

HanzoBot 可以为 HanzoBot 启动一个**专用、隔离的** Chrome/Brave/Edge/Chromium 实例并暴露一个小型 local loopback 控制服务。
...
- 配置文件颜色：`#FF4500`（龙虾橙）
- 注意：控制服务器由运行中的 Gateway 网关（HanzoBot.app 菜单栏或 `hanzo-bot gateway`）启动。
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/hanzo-bot" }
...
- 示例：`"/ui"`、`"/hanzo-bot"`、`"/apps/hanzo-bot"`。
- 默认：根路径（`/`）（不变）。
...
- 当连接来自这些 IP 之一时，HanzoBot 使用 `x-forwarded-for`（或 `x-real-ip`）来确定客户端 IP，用于本地配对检查和 HTTP 认证/本地检查。
- 仅列出你完全控制的代理，并确保它们**覆盖**传入的 `x-forwarded-for`。

说明：

- `hanzo-bot gateway` 拒绝启动，除非 `gateway.mode` 设为 `local`（或你传递了覆盖标志）。
...
- 优先级：`--port` > `BOT_GATEWAY_PORT` > `gateway.port` > 默认 `18789`。
...
Gmail 辅助配置（由 `hanzo-bot webhooks gmail setup` / `run` 使用）：
...
- 监视目录并通过 `/__bot__/ws` 的 WebSocket 端点广播重载
- 目录为空时自动创建起始 `index.html`（以便你立即看到内容）
- 同时在 `/__bot__/a2ui/` 提供 A2UI，并作为 `canvasHostUrl` 通告给节点
...
控制 LAN mDNS 发现广播（`_hanzo-bot-gw._tcp`）。

- `minimal`（默认）：从 TXT 记录中省略 `cliPath` + `sshPort`
- `full`：在 TXT 记录中包含 `cliPath` + `sshPort`
- `off`：完全禁用 mDNS 广播
- 主机名：默认为 `hanzo-bot`（通告 `hanzo-bot.local`）。通过 `BOT_MDNS_HOSTNAME` 覆盖。
