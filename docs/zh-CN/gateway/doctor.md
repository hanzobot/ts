...
`hanzo-bot doctor` 是 HanzoBot 的修复 + 迁移工具。它修复过时的配置/状态，检查健康状况，并提供可操作的修复步骤。
...
当配置包含已弃用的键时，其他命令会拒绝运行并要求你运行 `hanzo-bot doctor`。

Doctor 将：

- 解释找到了哪些遗留键。
- 显示它应用的迁移。
- 使用更新后的 schema 重写 `~/.hanzo-bot/hanzo-bot.json`。
...
- **多个状态目录**：当多个 `~/.hanzo-bot` 文件夹存在于不同 home 目录或当 `BOT_STATE_DIR` 指向别处时发出警告（历史可能在安装之间分裂）。
- **远程模式提醒**：如果 `gateway.mode=remote`，doctor 会提醒你在远程主机上运行它（状态在那里）。
- **配置文件权限**：当 `~/.hanzo-bot/hanzo-bot.json` 对组/其他用户可读时发出警告，并提供收紧到 `600` 的选项。
...
Doctor 检测遗留的 Gateway 网关服务（launchd/systemd/schtasks），并提供删除它们并使用当前 Gateway 网关端口安装 HanzoBot 服务的选项。它还可以扫描额外的类 Gateway 网关服务并打印清理提示。
配置文件命名的 HanzoBot Gateway 网关服务被视为一等公民，不会被标记为"额外的"。
...
当本地 Gateway 网关缺少 `gateway.auth` 时，Doctor 会发出警告并提供生成 token 的选项。使用 `hanzo-bot doctor --generate-gateway-token` 在自动化中强制创建 token。
...
- `hanzo-bot doctor` 在重写 supervisor 配置前提示。
- `hanzo-bot doctor --yes` 接受默认修复提示。
- `hanzo-bot doctor --repair` 无需提示应用推荐的修复。
- `hanzo-bot doctor --repair --force` 覆盖自定义 supervisor 配置。
- 你始终可以通过 `hanzo-bot gateway install --force` 强制完全重写。
