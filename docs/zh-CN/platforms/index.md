...
Hanzo Bot 核心使用 TypeScript 编写。**Node 是推荐的运行时**。
...
- 配置流程：`hanzo-bot configure` → 选择 **Gateway 服务**
- 修复/迁移：`hanzo-bot doctor`（会提供安装或修复服务的选项）

服务目标取决于操作系统：

- macOS：LaunchAgent（`ai.hanzo.bot.gateway` 或 `ai.hanzo.bot.<profile>`；旧版为 `com.openclaw.*`）
