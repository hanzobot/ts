...
- `~/Library/Application Support/HanzoBot/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `hanzo-bot-canvas://<session>/<path>`

示例：

- `hanzo-bot-canvas://main/` → `<canvasRoot>/main/index.html`
- `hanzo-bot-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `hanzo-bot-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```js
window.location.href = "hanzo-bot://agent?message=Review%20this%20design";
```
