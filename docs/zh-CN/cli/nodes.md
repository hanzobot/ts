...
hanzo-bot nodes list --connected
hanzo-bot nodes list --last-connected 24h
hanzo-bot nodes pending
hanzo-bot nodes approve <requestId>
hanzo-bot nodes status
hanzo-bot nodes status --connected
hanzo-bot nodes status --last-connected 24h
```
...
```bash
hanzo-bot nodes invoke --node <id|name|ip> --command <command> --params <json>
hanzo-bot nodes run --node <id|name|ip> <command...>
hanzo-bot nodes run --raw "git status"
hanzo-bot nodes run --agent main --node <id|name|ip> --raw "git status"
```
