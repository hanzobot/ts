...
read_when:
  - 添加或修改 CLI 命令或选项
  - 为新命令界面编写文档
summary: HanzoBot `hanzo-bot` 命令、子命令和选项的 CLI 参考
title: CLI 参考
x-i18n:
...
注意：插件可以添加额外的顶级命令（例如 `hanzo-bot voicecall`）。

## 安全

- `hanzo-bot security audit` — 审计配置 + 本地状态中常见的安全隐患。
- `hanzo-bot security audit --deep` — 尽力进行实时 Gateway 网关探测。
- `hanzo-bot security audit --fix` — 收紧安全默认值并 chmod 状态/配置。

## 插件

管理扩展及其配置：

- `hanzo-bot plugins list` — 发现插件（使用 `--json` 获取机器可读输出）。
...
- `hanzo-bot memory status` — 显示索引统计。
- `hanzo-bot memory index` — 重新索引记忆文件。
- `hanzo-bot memory search "<query>"` — 对记忆进行语义搜索。
...
非交互式配置辅助工具（get/set/unset）。不带子命令运行 `hanzo-bot config` 会启动向导。
...
- `channels status`：检查 Gateway 网关可达性和渠道健康状况（`--probe` 运行额外检查；使用 `hanzo-bot health` 或 `hanzo-bot status --deep` 进行 Gateway 网关健康探测）。
- 提示：`channels status` 在检测到常见配置错误时会打印带有建议修复的警告（然后指向 `hanzo-bot doctor`）。
...
当 OAuth/API 凭证可用时，HanzoBot 可以显示提供商用量/配额。
...
- `gateway status` 在检测到旧版或额外的 Gateway 网关服务时也会显示（`--deep` 添加系统级扫描）。配置文件命名的 HanzoBot 服务被视为一等公民，不会被标记为"额外"。
...
`node` 运行**无头节点主机**或将其作为后台服务管理。参见 [`hanzo-bot node`](/cli/node)。
...
浏览器控制 CLI（专用 Chrome/Brave/Edge/Chromium）。参见 [`hanzo-bot browser`](/cli/browser) 和[浏览器工具](/tools/browser)。
