...
Hanzo Bot 包含三个 Vitest 测试套件（单元/集成、端到端、实时）以及一小组 Docker 运行器。
...
- Gemini API 与 Gemini CLI：
  - API：Hanzo Bot 通过 HTTP 调用 Google 托管的 Gemini API（API 密钥/配置文件认证）；这是大多数用户说的"Gemini"。
  - CLI：Hanzo Bot 调用本地 `gemini` 二进制文件；它有自己的认证，行为可能不同（流式传输/工具支持/版本差异）。
...
- 配置：`~/.hanzoai/bot.json`（或 `BOT_CONFIG_PATH`）
...
- `BOT_CONFIG_DIR=...`（默认：`~/.openclaw`）挂载到 `/home/node/.openclaw`
