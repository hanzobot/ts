...
OpenClaw 从 `$BOT_CONFIG_PATH`（默认：`~/.openclaw/openclaw.json`）读取可选的 **JSON5** 配置：
...
- 当前工作目录下的 `.env`
- `~/.openclaw/.env`（即 `$BOT_STATE_DIR/.env`）的全局回退 `.env`
...
- macOS：`$BOT_STATE_DIR/logs/gateway.log` 和 `gateway.err.log`（默认：`~/.openclaw/logs/...`；配置文件使用 `~/.openclaw-<profile>/logs/...`）
