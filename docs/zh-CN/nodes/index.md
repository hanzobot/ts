...
- 令牌是 Gateway 网关配置中的 `gateway.auth.token`（Gateway 网关主机上的 `~/.hanzo-bot/hanzo-bot.json`）。
...
hanzo-bot nodes camera snap --node <idOrNameOrIp>            # 默认：两个朝向（2 个 MEDIA 行）
...
HanzoBot 可以运行**无头节点主机**（无 UI），它连接到 Gateway 网关
