...
summary: 社区构建的基于 Hanzo Bot 的项目和集成
title: 案例展示
x-i18n:
...
OpenCode 完成更改 → 打开 PR → Hanzo Bot 审查差异并在 Telegram 中回复"小建议"加上明确的合并决定（包括需要先应用的关键修复）。
...
跨 Agents、Claude、Codex 和 Hanzo Bot 管理 skills/命令的桌面应用。
...
<Card title="R2 上传 (Send Me My Files)" icon="cloud-arrow-up" href="https://skills.hanzo.bot/skills/r2-upload">
  **@julianengel** • `files` `r2` `presigned-urls`

上传到 Cloudflare R2/S3 并生成安全的预签名下载链接。非常适合远程 Hanzo Bot 实例。
</Card>
...
与智能体工作流（Claude Code、Hanzo Bot）集成的 Linear CLI。从终端管理问题、项目和工作流。首个外部 PR 已合并！
</Card>
...
Claude Code 发现并确认了净化器控制，然后 Hanzo Bot 接管来管理房间空气质量。
...
由屋顶摄像头触发：让 Hanzo Bot 在天空看起来很美的时候拍一张照片——它设计了一个 skill 并拍摄了照片。
...
定时提示每天早上通过 Hanzo Bot 角色生成一张"场景"图片（天气、任务、日期、喜欢的帖子/引言）。
</Card>
...
Hanzo Bot 连接到 Jira，然后即时生成一个新的 skill（在它出现在 ClawHub 之前）。
</Card>
...
自动化 Todoist 任务，并让 Hanzo Bot 直接在 Telegram 聊天中生成 skill。
</Card>
...
<Card title="xuezh 中文学习" icon="language" href="https://github.com/joshp123/xuezh">
  **@joshp123** • `learning` `voice` `skill`
  ...
  通过 Hanzo Bot 实现带有发音反馈和学习流程的中文学习引擎。
...
<Card title="Home Assistant 插件" icon="home" href="https://github.com/ngutman/openclaw-ha-addon">
  **@ngutman** • `homeassistant` `docker` `raspberry-pi`
  ...
  在 Home Assistant OS 上运行的 Hanzo Bot Gateway 网关，支持 SSH 隧道和持久状态。
</Card>
...
<Card title="GoHome 自动化" icon="house-signal" href="https://github.com/joshp123/gohome">
  **@joshp123** • `home` `nix` `grafana`
  ...
  Nix 原生家庭自动化，以 Hanzo Bot 作为界面，加上漂亮的 Grafana 仪表板。
