...
summary: 设置指南：在保持最新的同时保持你的 HanzoBot 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.hanzo-bot/workspace`（工作区）+ `~/.hanzo-bot/hanzo-bot.json`（配置）。
...
- **配置：** `~/.hanzo-bot/hanzo-bot.json`（JSON/JSON5 格式）
...
如果你还没有全局安装，通过 `pnpm hanzo-bot setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **HanzoBot.app**（菜单栏）。
...
- 运行 `hanzo-bot setup`，然后 `hanzo-bot channels login`，然后手动启动 Gateway 网关（`hanzo-bot gateway`）。
...
- 将 `~/.hanzo-bot/workspace` 和 `~/.hanzo-bot/` 保持为"你的东西"；不要将个人提示/配置放入 `hanzo-bot` 仓库。
