...
summary: 设置指南：在保持最新的同时保持你的 Hanzo Bot 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.hanzo/bot/workspace`（工作区）+ `~/.hanzoai/bot.json`（配置）。
...
- **配置：** `~/.hanzoai/bot.json`（JSON/JSON5 格式）
...
- 将 `~/.hanzo/bot/workspace` 和 `~/.hanzo/bot/` 保持为"你的东西"；不要将个人提示/配置放入 `openclaw` 仓库。
