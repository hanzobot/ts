...
你可以在 `~/.hanzo-bot/hanzo-bot.json` 中覆盖此设置：
...
所有日志配置都在 `~/.hanzo-bot/hanzo-bot.json` 的 `logging` 下。
...
- HanzoBot 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `hanzo-bot plugins enable diagnostics-otel` 启用插件。
...
- `hanzo-bot.tokens`（计数器，属性：`hanzo-bot.token`、`hanzo-bot.channel`、`hanzo-bot.provider`、`hanzo-bot.model`）
- `hanzo-bot.cost.usd`（计数器，属性：`hanzo-bot.channel`、`hanzo-bot.provider`、`hanzo-bot.model`）
- `hanzo-bot.run.duration_ms`（直方图，属性：`hanzo-bot.channel`、`hanzo-bot.provider`、`hanzo-bot.model`）
- `hanzo-bot.context.tokens`（直方图，属性：`hanzo-bot.context`、`hanzo-bot.channel`、`hanzo-bot.provider`、`hanzo-bot.model`）

消息流：

- `hanzo-bot.webhook.received`（计数器，属性：`hanzo-bot.channel`、`hanzo-bot.webhook`）
- `hanzo-bot.webhook.error`（计数器，属性：`hanzo-bot.channel`、`hanzo-bot.webhook`）
- `hanzo-bot.webhook.duration_ms`（直方图，属性：`hanzo-bot.channel`、`hanzo-bot.webhook`）
- `hanzo-bot.message.queued`（计数器，属性：`hanzo-bot.channel`、`hanzo-bot.source`）
- `hanzo-bot.message.processed`（计数器，属性：`hanzo-bot.channel`、`hanzo-bot.outcome`）
- `hanzo-bot.message.duration_ms`（直方图，属性：`hanzo-bot.channel`、`hanzo-bot.outcome`）

队列 + 会话：

- `hanzo-bot.queue.lane.enqueue`（计数器，属性：`hanzo-bot.lane`）
- `hanzo-bot.queue.lane.dequeue`（计数器，属性：`hanzo-bot.lane`）
- `hanzo-bot.queue.depth`（直方图，属性：`hanzo-bot.lane` 或 `hanzo-bot.channel=heartbeat`）
- `hanzo-bot.queue.wait_ms`（直方图，属性：`hanzo-bot.lane`）
- `hanzo-bot.session.state`（计数器，属性：`hanzo-bot.state`、`hanzo-bot.reason`）
- `hanzo-bot.session.stuck`（计数器，属性：`hanzo-bot.state`）
- `hanzo-bot.session.stuck_age_ms`（直方图，属性：`hanzo-bot.state`）
- `hanzo-bot.run.attempt`（计数器，属性：`hanzo-bot.attempt`）
...
- `hanzo-bot.webhook.error`
  - `hanzo-bot.channel`、`hanzo-bot.webhook`、`hanzo-bot.chatId`、`hanzo-bot.error`
- `hanzo-bot.message.processed`
  - `hanzo-bot.channel`、`hanzo-bot.outcome`、`hanzo-bot.chatId`、`hanzo-bot.messageId`、`hanzo-bot.sessionKey`、`hanzo-bot.sessionId`、`hanzo-bot.reason`
- `hanzo-bot.session.stuck`
  - `hanzo-bot.state`、`hanzo-bot.ageMs`、`hanzo-bot.queueDepth`、`hanzo-bot.sessionKey`、`hanzo-bot.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `hanzo-bot doctor`。
