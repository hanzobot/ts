...
CLI onboarding is the **recommended** way to set up Hanzo Bot on macOS,
...
`hanzo-bot configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
If the config is invalid or contains legacy keys, onboarding asks you to run `hanzo-bot doctor` first.
</Note>
