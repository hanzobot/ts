...
- **Tailoring lives outside the repo:** `~/.hanzo/bot/workspace` (workspace) + `~/.hanzoai/bot.json` (config).
...
- Keep `~/.hanzo/bot/workspace` and `~/.hanzo/bot/` as “your stuff”; don’t put personal prompts/config into the `openclaw` repo.
...
- [Hanzo Bot assistant setup](/start/openclaw)
