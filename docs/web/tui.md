...
- Turn delivery on:
  ...
  - or start with `hanzo-bot tui --deliver`
...
- If your terminal uses a light background and auto-detection is wrong, set `BOT_THEME=light` before launching `hanzo-bot tui`.
...
- Check the Gateway logs: `hanzo-bot logs --follow`.
- Confirm the agent can run: `hanzo-bot status` and `hanzo-bot models status`.
...
- No agents in picker: check `hanzo-bot agents list` and your routing config.
