...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/hanzo-bot`)
...
If the page fails to load, start the Gateway first: `hanzo-bot gateway`.
...
you revoke it with `hanzo-bot devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.hanzo-bot/hanzo-bot.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop hanzo-bot`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. HanzoBot
...
HanzoBot **blocks** Control UI connections without device identity.
