...
- Pool note:
  - Hanzo Bot uses Vitest `vmForks` on Node 22, 23, and 24 for faster unit shards.
  - On Node 25+, Hanzo Bot automatically falls back to regular `forks` until the repo is re-validated there.
...
- Scope:
  ...
  - Exercises Hanzo Bot's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  - `BOT_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `BOT_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
...
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `BOT_LIVE_*_KEY`; tests retry on rate limit responses.
...
- How to select models:
  - `BOT_LIVE_MODELS=modern` to run the modern allowlist (Opus/Sonnet/Haiku 4.5, GPT-5.x + Codex, Gemini 3, GLM 4.7, MiniMax M2.5, Grok 4)
...
- Overrides (optional):
  ...
  - `BOT_LIVE_CLI_BACKEND_MODEL="codex-cli/gpt-5.4"`
...
- Tool calling across several providers:
  - `BOT_LIVE_GATEWAY_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,google/gemini-3-flash-preview,zai/glm-4.7,minimax/minimax-m2.5" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
`BOT_LIVE_GATEWAY_MODELS="openai/gpt-5.2,openai-codex/gpt-5.4,anthropic/claude-opus-4-6,google/gemini-3.1-pro-preview,google/gemini-3-flash-preview,google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-flash,zai/glm-4.7,minimax/minimax-m2.5" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
- Config: `~/.hanzoai/bot.json` (or `BOT_CONFIG_PATH`)
...
- Current bundled providers covered:
  ...
  - `google`
- Optional narrowing:
  - `BOT_LIVE_IMAGE_GENERATION_PROVIDERS="openai,google"`
  - `BOT_LIVE_IMAGE_GENERATION_MODELS="openai/gpt-image-1,google/gemini-3.1-flash-image-preview"`
  - `BOT_LIVE_IMAGE_GENERATION_CASES="google:flash-generate,google:pro-edit"`
- Optional auth behavior:
  - `BOT_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
`BOT_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
...
- `BOT_CONFIG_DIR=...` (default: `~/.openclaw`) mounted to `/home/node/.openclaw`
...
- `BOT_LIVE_GATEWAY_PROVIDERS=...` / `BOT_LIVE_PROVIDERS=...` to filter providers in-container
