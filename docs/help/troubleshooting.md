---
summary: "Symptom first troubleshooting hub for HanzoBot"
read_when:
  - HanzoBot is not working and you need the fastest path to a fix
...
If install fails with `package.json missing hanzo-bot.extensions`, the plugin package
is using an old shape that HanzoBot no longer accepts.

Fix in the plugin package:

1. Add `hanzo-bot.extensions` to `package.json`.
...
```mermaid
flowchart TD
  A[HanzoBot is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Browser tool fails">
    ...
    - `hanzo-bot` profile starts or `chrome` relay has an attached tab.
