...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be managed via CLI commands, similar to how skills work in HanzoBot.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in HanzoBot. See [Webhook Hooks](/automation/webhook) or use `hanzo-bot webhooks` for Gmail helper commands.
...
- Extend HanzoBot's behavior without modifying core code
...
Hook packs are standard npm packages that export one or more hooks via `hanzo-bot.hooks` in
...
Security note: `hanzo-bot hooks install` installs dependencies with `npm install --ignore-scripts`
...
homepage: https://docs.hanzo-bot.ai/automation/hooks#my-hook
metadata:
...
The `metadata.hanzo-bot` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before HanzoBot persists them.
...
```bash
hanzo-bot hooks enable bootstrap-extra-files
```
...
```yaml
metadata: { "hanzo-bot": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "hanzo-bot": { "events": ["command"] } } # General - more overhead
```
...
- [Bundled Hooks README](https://github.com/hanzo-bot/hanzo-bot/tree/main/src/hooks/bundled)
