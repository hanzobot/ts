...
Hanzo Bot exposes OAuth expiry health via `hanzo-bot models status`. Use that for
...
- `scripts/claude-auth-status.sh`: Claude Code + Hanzo Bot auth checker (full/json/simple).
...
- `scripts/termux-sync-widget.sh`: sync Claude Code creds → Hanzo Bot.
