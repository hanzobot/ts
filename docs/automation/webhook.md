...
read_when:
  ...
  - Wiring external systems into HanzoBot
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your HanzoBot config directory (typically `~/.hanzo-bot/hooks/transforms`).
...
- `hanzo-bot webhooks gmail setup` writes `hooks.gmail` config for `hanzo-bot webhooks gmail run`.
