...
To reduce top-of-hour load spikes across many gateways, HanzoBot applies a
...
If `delivery` is omitted for isolated jobs, HanzoBot defaults to `announce`.
...
When a job fails, HanzoBot classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, HanzoBot archives transcript files and purges old deleted archives on the same retention window.
...
```bash
hanzo-bot cron edit <jobId> --exact
```
...
- HanzoBot applies exponential retry backoff for recurring jobs after consecutive errors:
