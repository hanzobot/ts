...
- CLI helpers: `hanzo-bot onboard`, `hanzo-bot models list`, `hanzo-bot models set <provider/model>`.
- Provider plugins can inject model catalogs via `registerProvider({ catalog })`;
  Hanzo Bot merges that output into `models.providers` before writing
  `models.json`.
...
Provider plugins can now own most provider-specific logic while Hanzo Bot keeps
the generic inference loop.

Typical split:

- `auth[].run` / `auth[].runNonInteractive`: provider owns onboarding/login
  flows for `hanzo-bot onboard`, `hanzo-bot models auth`, and headless setup
...
That covers providers that still fit Hanzo Bot's normal transports. A provider
...
- Configure multiple keys via:
  - `BOT_LIVE_<PROVIDER>_KEY` (single live override, highest priority)
...
Hanzo Bot ships with the pi‑ai catalog. These providers require **no**
...
- `openai/gpt-5.3-codex-spark` is intentionally suppressed in Hanzo Bot because the live OpenAI API rejects it; Spark is treated as Codex-only
...
- Optional rotation: `ANTHROPIC_API_KEYS`, `ANTHROPIC_API_KEY_1`, `ANTHROPIC_API_KEY_2`, plus `BOT_LIVE_ANTHROPIC_KEY` (single override)
- Example model: `anthropic/claude-opus-4-6`
- CLI: `hanzo-bot onboard --auth-choice token` (paste setup-token) or `hanzo-bot models auth paste-token --provider anthropic`
- Direct API-key models support the shared `/fast` toggle and `params.fastMode`; Hanzo Bot maps that to Anthropic `service_tier` (`auto` vs `standard_only`)
...
- Policy note: OpenAI Codex OAuth is explicitly supported for external tools/workflows like Hanzo Bot.
...
- CLI: `hanzo-bot onboard --auth-choice opencode-zen` or `hanzo-bot onboard --auth-choice opencode-go`
...
- Provider: `google`
- Auth: `GEMINI_API_KEY`
- Optional rotation: `GEMINI_API_KEYS`, `GEMINI_API_KEY_1`, `GEMINI_API_KEY_2`, `GOOGLE_API_KEY` fallback, and `BOT_LIVE_GEMINI_KEY` (single override)
- Example models: `google/gemini-3.1-pro-preview`, `google/gemini-3-flash-preview`
- Compatibility: legacy Hanzo Bot config using `google/gemini-3.1-flash-preview` is normalized to `google/gemini-3-flash-preview`
- CLI: `hanzo-bot onboard --auth-choice gemini-api-key`

### Google Vertex and Gemini CLI

- Providers: `google-vertex`, `google-gemini-cli`
- Auth: Vertex uses gcloud ADC; Gemini CLI uses its OAuth flow
- Caution: Gemini CLI OAuth in Hanzo Bot is an unofficial integration. Some users have reported Google account restrictions after using third-party clients. Review Google terms and use a non-critical account if you choose to proceed.
- Gemini CLI OAuth is shipped as part of the bundled `google` plugin.
  - Enable: `hanzo-bot plugins enable google`
...
- CLI: `hanzo-bot onboard --auth-choice zai-api-key`
...
- CLI: `hanzo-bot onboard --auth-choice ai-gateway-api-key`
...
- CLI: `hanzo-bot onboard --kilocode-api-key <key>`
...
- CLI: `hanzo-bot onboard --auth-choice mistral-api-key`
- Groq: `groq` (`GROQ_API_KEY`)
- Cerebras: `cerebras` (`CEREBRAS_API_KEY`)
  - GLM models on Cerebras use ids `zai-glm-4.7` and `zai-glm-4.6`.
  - OpenAI-compatible base URL: `https://api.cerebras.ai/v1`.
- GitHub Copilot: `github-copilot` (`COPILOT_GITHUB_TOKEN` / `GH_TOKEN` / `GITHUB_TOKEN`)
- Hugging Face Inference example model: `huggingface/deepseek-ai/DeepSeek-R1`; CLI: `hanzo-bot onboard --auth-choice huggingface-api-key`. See [Hugging Face (Inference)](/providers/huggingface).
...
- CLI: `hanzo-bot onboard --auth-choice volcengine-api-key`
...
- CLI: `hanzo-bot onboard --auth-choice byteplus-api-key`
...
- CLI: `hanzo-bot onboard --auth-choice synthetic-api-key`
...
`hanzo-bot onboard` and the model picker. See [/providers/ollama](/providers/ollama)
...
- For custom providers, `reasoning`, `input`, `cost`, `contextWindow`, and `maxTokens` are optional.
  When omitted, Hanzo Bot defaults to:
  - `reasoning: false`
  - `input: ["text"]`
  - `cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }`
  - `contextWindow: 200000`
...
- For `api: "openai-completions"` on non-native endpoints (any non-empty `baseUrl` whose host is not `api.openai.com`), Hanzo Bot forces `compat.supportsDeveloperRole: false` to avoid provider 400 errors for unsupported `developer` roles.
- If `baseUrl` is empty/omitted, Hanzo Bot keeps the default OpenAI behavior (which resolves to `api.openai.com`).
