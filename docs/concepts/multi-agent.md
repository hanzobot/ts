...
If you do nothing, HanzoBot runs a single agent:
...
```bash
hanzo-bot agents list --bindings
```
...
```bash
hanzo-bot agents add coding
hanzo-bot agents add social
```

Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.hanzo-bot/agents/<agentId>`.
...
- If you later add the same binding for the same agent with an explicit account id, HanzoBot upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, HanzoBot falls back
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.hanzo-bot/workspace-main" },
      { id: "coding", workspace: "~/.hanzo-bot/workspace-coding" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.hanzo-bot/workspace-main" },
      { id: "alerts", workspace: "~/.hanzo-bot/workspace-alerts" },
    ],
  },
  bindings: [
...
```bash
hanzo-bot channels login --channel whatsapp --account personal
hanzo-bot channels login --channel whatsapp --account biz
```

`~/.hanzo-bot/hanzo-bot.json` (JSON5):
