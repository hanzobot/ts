...
- State dir: `~/.openclaw` (or `BOT_STATE_DIR`)
...
If you do nothing, Hanzo Bot runs a single agent:
...
- If you later add the same binding for the same agent with an explicit account id, Hanzo Bot upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, Hanzo Bot falls back
