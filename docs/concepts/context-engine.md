...
read_when:
  - You want to understand how Hanzo Bot assembles model context
...
A **context engine** controls how Hanzo Bot builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Hanzo Bot ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.hanzoai/bot.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Hanzo Bot plugin. Install
...
Every time Hanzo Bot runs a model prompt, the context engine participates at
...
Hanzo Bot currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Hanzo Bot
...
The built-in `legacy` engine preserves Hanzo Bot's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Hanzo Bot uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Hanzo Bot disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Hanzo Bot automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  ...
  Hanzo Bot's built-in compaction behavior.
...
Hanzo Bot resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Hanzo Bot's built-in summarization. Plugin engines can implement
...
- Use `hanzo-bot doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Hanzo Bot
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `hanzo-bot plugins install -l ./my-engine` to link a
