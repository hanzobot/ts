...
- `/usage cost` in chats: local cost summary aggregated from Hanzo Bot session logs.
- CLI: `hanzo-bot status --usage` prints a full per-provider breakdown.
- CLI: `hanzo-bot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
