---
summary: "Context window + compaction: how Hanzo Bot keeps sessions under model limits"
read_when:
...
Every model has a **context window** (max tokens it can see). Long-running chats accumulate messages and tool results; once the window is tight, Hanzo Bot **compacts** older history to stay within limits.
...
When a session nears or exceeds the model’s context window, Hanzo Bot triggers auto-compaction and may retry the original request using the compacted context.
...
Before compaction, Hanzo Bot can run a **silent memory flush** turn to store
...
Context window is model-specific. Hanzo Bot uses the model definition from the configured provider catalog to determine limits.
...
When a plugin engine sets `ownsCompaction: true`, Hanzo Bot delegates all
compaction decisions to the engine and does not run built-in auto-compaction.

When `ownsCompaction` is `false` or unset, Hanzo Bot may still use Pi's
