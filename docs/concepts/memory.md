---
title: "Memory"
summary: "How HanzoBot memory works (workspace files + automatic memory flush)"
read_when:
...
HanzoBot memory is **plain Markdown in the agent workspace**. The files are the
...
HanzoBot exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, HanzoBot triggers a **silent,
...
HanzoBot can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses remote embeddings by default. If `memorySearch.provider` is not set, HanzoBot auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. HanzoBot
...
BM25 + vectors + reranking. Markdown stays the source of truth; HanzoBot shells
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, HanzoBot retries with `qmd query`. If QMD fails or the binary is
  missing, HanzoBot automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- HanzoBot does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - HanzoBot sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
...
- When `scope` denies a search, HanzoBot logs a warning with the derived
...
When enabled, HanzoBot combines:

- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, HanzoBot falls back to vector-only search.
...
HanzoBot can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
When the sqlite-vec extension is available, HanzoBot stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, HanzoBot logs the
