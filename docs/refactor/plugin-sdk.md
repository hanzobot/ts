...
- Publish as `hanzo-bot/plugin-sdk` (or export from core under `hanzo-bot/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `HanzoBotPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: HanzoBotConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: HanzoBotConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: HanzoBotConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: HanzoBotConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: HanzoBotConfig): string;
  };
};
```
...
- Introduce `hanzo-bot/plugin-sdk`.
- Add `api.runtime` to `HanzoBotPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `botRuntime: ">=2026.2.0"`).
