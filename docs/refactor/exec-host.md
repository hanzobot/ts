...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.hanzo-bot/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.hanzo-bot/node.json` (node id + pairing token).
...
Path: `~/.hanzo-bot/exec-approvals.json`
...
- Unix socket at `~/.hanzo-bot/exec-approvals.sock` (0600).
