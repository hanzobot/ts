#!/usr/bin/env bash
# One-time host setup for rootless Hanzo Bot in Podman: creates the openclaw
...
# Or as the hanzo-bot user: sudo -u hanzo-bot /home/openclaw/run-openclaw-podman.sh
...
# Create hanzo-bot user (non-login, with home) if missing
...
BUILD_ARGS=()
[[ -n "${BOT_DOCKER_APT_PACKAGES:-}" ]] && BUILD_ARGS+=(--build-arg "BOT_DOCKER_APT_PACKAGES=${BOT_DOCKER_APT_PACKAGES}")
[[ -n "${BOT_EXTENSIONS:-}" ]] && BUILD_ARGS+=(--build-arg "BOT_EXTENSIONS=${BOT_EXTENSIONS}")
...
# do not need to be traversable by $BOT_USER.
cat "$TMP_IMAGE" | run_as_user "$BOT_USER" env HOME="$BOT_HOME" podman load
rm -rf "$TMP_STAGE_DIR"
trap - EXIT
...
# Optionally install systemd quadlet for hanzo-bot user (rootless Podman + systemd)
