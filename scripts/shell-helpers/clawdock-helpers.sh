#!/usr/bin/env bash
# ClawDock - Docker helpers for HanzoBot
# Inspired by Simon Willison's "Running HanzoBot in Docker"
...
# Common paths to check for HanzoBot
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found HanzoBot at: $found_path"
    ...
    echo ""
    echo "❌ HanzoBot not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/hanzo-bot/hanzo-bot.git ~/hanzo-bot"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d hanzo-bot-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart hanzo-bot-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f hanzo-bot-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec hanzo-bot-gateway \
...
clawdock-cli() {
  _clawdock_compose run --rm hanzo-bot-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build hanzo-bot-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "BOT_GATEWAY_TOKEN=$token" hanzo-bot-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" hanzo-bot-gateway \
    bash -c './hanzo-bot.mjs config set gateway.remote.token "$TOKEN" && ./hanzo-bot.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec hanzo-bot-gateway \
    bash -c "./hanzo-bot.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart hanzo-bot-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm hanzo-bot-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec hanzo-bot-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for HanzoBot${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (hanzo-bot alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to hanzo-bot project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.hanzo-bot)${_CLR_RESET}"
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.hanzo-bot.ai${_CLR_RESET}"
  echo ""
}
