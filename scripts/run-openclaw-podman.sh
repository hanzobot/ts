#!/usr/bin/env bash
# Rootless Hanzo Bot in Podman: run after one-time setup.
#
...
#
# As the hanzo-bot user (no repo needed):
...
LAUNCH_SCRIPT="$BOT_HOME/run-openclaw-podman.sh"
...
# --- Step 2: launch (from repo: re-exec as hanzo-bot in safe cwd; from hanzo-bot home: run container) ---
if [[ "${1:-}" == "launch" ]]; then
  shift
  if [[ -n "${BOT_UID:-}" && "$(id -u)" -ne "$BOT_UID" ]]; then
    # Exec as hanzo-bot with cwd=/tmp so a nologin user never inherits an invalid cwd.
...
# --- Container run (script in hanzo-bot home, run as openclaw) ---
EFFECTIVE_HOME="${HOME:-}"
...
CONFIG_DIR="${BOT_CONFIG_DIR:-$EFFECTIVE_HOME/.openclaw}"
ENV_FILE="${BOT_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${BOT_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${BOT_PODMAN_CONTAINER:-openclaw}"
BOT_IMAGE="${BOT_PODMAN_IMAGE:-openclaw:local}"
...
# NOTE: must be evaluated after sourcing ENV_FILE so BOT_GATEWAY_BIND set in .env takes effect.
GATEWAY_BIND="${BOT_GATEWAY_BIND:-loopback}"
...
SELINUX_MOUNT_OPTS=""
if [[ -z "${BOT_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  # Honour explicit override (e.g. BOT_BIND_MOUNT_OPTIONS=":Z" → strip leading colon for inline use).
  SELINUX_MOUNT_OPTS="${BOT_BIND_MOUNT_OPTIONS#:}"
...
if [[ "$RUN_SETUP" == true ]]; then
  exec podman run --pull="$PODMAN_PULL" --rm -it \
    ...
    -v "$WORKSPACE_DIR:/home/node/.hanzo/bot/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "${ENV_FILE_ARGS[@]}" \
    "$BOT_IMAGE" \
