...
IMAGE_NAME="hanzo-bot-plugins-e2e"
...
echo "Running plugins Docker E2E..."
	docker run --rm -t "$IMAGE_NAME" bash -lc '
	  ...
	  home_dir=$(mktemp -d "/tmp/hanzo-bot-plugins-e2e.XXXXXX")
...
JS
  cat > "$HOME/.hanzo-bot/extensions/demo-plugin/hanzo-bot.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/hanzo-bot-plugin-pack.XXXXXX")"
...
JS
  cat > "$pack_dir/package/hanzo-bot.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/hanzo-bot-plugin-dir.XXXXXX")"
...
JS
  cat > "$dir_plugin/hanzo-bot.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/hanzo-bot-plugin-filepack.XXXXXX")"
...
JS
  cat > "$file_pack_dir/package/hanzo-bot.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
