...
IMAGE_NAME="hanzo-bot-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/hanzo-bot-bin:$PATH"
  mkdir -p /tmp/hanzo-bot-bin
  cat > /tmp/hanzo-bot-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/hanzo-bot-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$BOT_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/hanzo-bot-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/hanzo-bot-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$BOT_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/hanzo-bot-e2e-$1.XXXXXX"
  }
  ...
  set_isolated_bot_env() {
    ...
    export BOT_STATE_DIR="$home_dir/.hanzo-bot"
    export BOT_CONFIG_PATH="$BOT_STATE_DIR/hanzo-bot.json"
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$BOT_STATE_DIR/workspace"
    config_path="$BOT_CONFIG_PATH"
    sessions_dir="$BOT_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$BOT_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$BOT_ENTRY\" configure --section skills" send_skills_flow
