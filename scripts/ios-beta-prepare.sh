...
- regenerates apps/ios/Hanzo Bot.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${BOT_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid BOT_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid BOT_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid BOT_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid BOT_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing BOT_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(BOT_URL_SLASH)$(BOT_URL_SLASH)#g'
)"
...
BOT_CODE_SIGN_STYLE = Automatic
BOT_DEVELOPMENT_TEAM = ${TEAM_ID}
BOT_IOS_SELECTED_TEAM = ${TEAM_ID}
BOT_APP_BUNDLE_ID = ai.hanzo.bot.client
BOT_SHARE_BUNDLE_ID = ai.hanzo.bot.client.share
BOT_ACTIVITY_WIDGET_BUNDLE_ID = ai.hanzo.bot.client.activitywidget
BOT_WATCH_APP_BUNDLE_ID = ai.hanzo.bot.client.watchkitapp
BOT_WATCH_EXTENSION_BUNDLE_ID = ai.hanzo.bot.client.watchkitapp.extension
BOT_APP_PROFILE =
BOT_SHARE_PROFILE =
BOT_PUSH_TRANSPORT = relay
BOT_PUSH_DISTRIBUTION = official
BOT_URL_SLASH = /
BOT_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
BOT_PUSH_APNS_ENVIRONMENT = production
EOF
