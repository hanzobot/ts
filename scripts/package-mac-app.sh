...
# Build and bundle HanzoBot into a minimal .app we can open.
# Outputs to dist/HanzoBot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/HanzoBot.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="HanzoBot"
BUNDLE_ID="${BUNDLE_ID:-ai.hanzo.bot.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/hanzo-bot/hanzo-bot/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/HanzoBot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :HanzoBotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :HanzoBotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/HanzoBot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/HanzoBot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/HanzoBot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/HanzoBot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/HanzoBot/Resources/HanzoBot.icns" "$APP_ROOT/Contents/Resources/HanzoBot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/HanzoBot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "⏹  Stopping any running HanzoBot"
killall -q HanzoBot 2>/dev/null || true
