...
OPENCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/Hanzo BotKit_Hanzo BotKit.bundle"
if [ -d "$OPENCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/Hanzo BotKit_Hanzo BotKit.bundle"
  cp -R "$OPENCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/Hanzo BotKit_Hanzo BotKit.bundle"
else
  echo "WARN: Hanzo BotKit resource bundle not found at $OPENCLAWKIT_BUNDLE (continuing)" >&2
fi
