# HanzoBot Installer for Windows (PowerShell)
# Usage: iwr -useb https://hanzo-bot.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://hanzo-bot.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\hanzo-bot",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Install-HanzoBotGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing HanzoBot from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/hanzo-bot/hanzo-bot.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\hanzo-bot\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\hanzo-bot.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install HanzoBot from git to $GitDir" -Level info
        } else {
            Install-HanzoBotGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install HanzoBot via npm (tag: $Tag)" -Level info
        } else {
            if (!(Install-HanzoBotNpm -Version $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'hanzo-bot onboard' to complete setup" -Level info
    }
