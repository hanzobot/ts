...
#
# Suggested cron: */30 * * * * /home/admin/hanzo-bot/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send HanzoBot notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., hanzo-bot-alerts)
...
STATE_FILE="$HOME/.hanzo-bot/auth-monitor-state"
...
send_notification() {
    ...
    # Send via HanzoBot if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use hanzo-bot
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via HanzoBot to $NOTIFY_PHONE..."
            hanzo-bot send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! HanzoBot is down. Run: ssh l36 '~/hanzo-bot/scripts/mobile-reauth.sh'" "urgent"
    exit 1
