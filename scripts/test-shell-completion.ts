 ...
 * `hanzo-bot update`. Use it to verify the completion installation flow
...
\`hanzo-bot update\`, \`hanzo-bot doctor\`, and \`hanzo-bot onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Hanzo Bot Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`hanzo-bot completion --install\` later to enable.`));
    return;
  }
