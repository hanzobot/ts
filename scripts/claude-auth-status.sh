...
# Checks both Claude Code and HanzoBot auth status
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    bot_status=$(check_bot_auth 2>/dev/null || true)

    claude_expires=0
    bot_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        bot_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        bot_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$BOT_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            hanzo-bot: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$bot_status" == EXPIRED* ]] || [[ "$bot_status" == MISSING* ]]; then
        echo "BOT_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$bot_status" == EXPIRING* ]]; then
        echo "BOT_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "HanzoBot Auth (~/.hanzo-bot/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'hanzo-bot doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
if systemctl --user is-active hanzo-bot >/dev/null 2>&1; then
    echo -e "HanzoBot service: ${GREEN}running${NC}"
else
    echo -e "HanzoBot service: ${RED}NOT running${NC}"
fi
