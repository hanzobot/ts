...
# HanzoBot Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://hanzo-bot.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one HanzoBot."
NODE_MIN_MAJOR=22
NODE_MIN_MINOR=12
...
GUM_VERSION="${BOT_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 HanzoBot Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://hanzo-bot.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$BOT_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.hanzo-bot.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_bot_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/hanzo-bot" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/hanzo-bot"|"/usr/local/bin/hanzo-bot")
                ;;
            *)
                return 1
                ;;
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing hanzo-bot binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing HanzoBot package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_bot_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "HanzoBot npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_bot_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_bot_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "HanzoBot npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an hanzo-bot binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "BOT_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "BOT_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "BOT_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "BOT_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "BOT_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "BOT_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "BOT_BETA" "CLAWDBOT_BETA"
map_legacy_env "BOT_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "BOT_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "BOT_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "BOT_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "BOT_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "BOT_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${BOT_TAGLINE_INDEX:-}" ]]; then
        if [[ "${BOT_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((BOT_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${BOT_NO_ONBOARD:-0}
NO_PROMPT=${BOT_NO_PROMPT:-0}
DRY_RUN=${BOT_DRY_RUN:-0}
INSTALL_METHOD=${BOT_INSTALL_METHOD:-}
BOT_VERSION=${BOT_VERSION:-latest}
USE_BETA=${BOT_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/hanzo-bot"
GIT_DIR=${BOT_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${BOT_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${BOT_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${BOT_VERBOSE:-0}"
BOT_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
HanzoBot installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://hanzo-bot.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/hanzo-bot)
...
Environment variables:
  ...
  BOT_VERSION=latest|next|<semver>
  BOT_BETA=0|1
  BOT_GIT_DIR=...
  BOT_GIT_UPDATE=0|1
  BOT_NO_PROMPT=1
  ...
  BOT_NO_ONBOARD=1
  BOT_VERBOSE=1
  BOT_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://hanzo-bot.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://hanzo-bot.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://hanzo-bot.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected HanzoBot checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a HanzoBot source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://hanzo-bot.ai/install.sh | bash"
}
...
# Check for existing HanzoBot installation
check_existing_hanzo-bot() {
    if [[ -n "$(type -P hanzo-bot 2>/dev/null || true)" ]]; then
        ui_info "Existing HanzoBot installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make hanzo-bot show as \"command not found\" in new terminals."
...
warn_bot_not_found() {
    ui_warn "Installed, but hanzo-bot is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t hanzo-bot 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named hanzo-bot; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_bot_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P hanzo-bot 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/hanzo-bot" ]]; then
        echo "${npm_bin}/hanzo-bot"
        return 0
    fi
...
install_bot_from_git() {
    ...
    if [[ -d "$repo_dir/.git" ]]; then
        ...
        ui_info "Installing HanzoBot from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning HanzoBot" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building HanzoBot" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/hanzo-bot" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/hanzo-bot"
    ui_success "HanzoBot wrapper installed to \$HOME/.local/bin/hanzo-bot"
...
install_hanzo-bot() {
    ...
    resolved_version="$(npm view "${package_name}@${BOT_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing HanzoBot v${resolved_version}"
    else
        ui_info "Installing HanzoBot (${BOT_VERSION})"
    fi
    local install_spec=""
    if [[ "${BOT_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${BOT_VERSION}"
    fi
...
run_doctor() {
    ...
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (hanzo-bot not on PATH yet)"
        warn_bot_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${BOT_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.hanzo-bot/workspace-${profile}"
    else
        echo "${HOME}/.hanzo-bot/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${BOT_CONFIG_PATH:-$HOME/.hanzo-bot/hanzo-bot.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run hanzo-bot onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but hanzo-bot not on PATH; skipping onboarding"
        warn_bot_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run hanzo-bot onboard to retry"
        return
    }
}
...
resolve_bot_version() {
    local version=""
    local claw="${BOT_BIN:-}"
    if [[ -z "$claw" ]] && command -v hanzo-bot &> /dev/null; then
        claw="$(command -v hanzo-bot)"
    fi
    if [[ -n "$claw" ]]; then
...
main() {
    ...
    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found HanzoBot checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set BOT_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_hanzo-bot; then
        is_upgrade=true
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        if [[ -x "$HOME/.local/bin/hanzo-bot" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/hanzo-bot"
    ...
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 HanzoBot installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 HanzoBot installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/hanzo-bot"
        ui_kv "Update command" "hanzo-bot update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://hanzo-bot.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running hanzo-bot doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                BOT_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                BOT_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                BOT_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run hanzo-bot doctor and hanzo-bot plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run hanzo-bot onboard later"
        else
            local config_path="${BOT_CONFIG_PATH:-$HOME/.hanzo-bot/hanzo-bot.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (hanzo-bot not on PATH yet)"
                    warn_bot_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run hanzo-bot onboard to finish setup"
            return 0
        fi
    fi

    if command -v hanzo-bot &> /dev/null; then
        local claw="${BOT_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_bot_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (hanzo-bot daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if BOT_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: hanzo-bot daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${BOT_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
