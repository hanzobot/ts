...
show_usage() {
    cat << EOF
clawlog - HanzoBot Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View HanzoBot logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    HanzoBot logs flow through the macOS unified log (subsystem: ai.hanzo.bot).
