#!/usr/bin/env bash
# Reset Hanzo Bot like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${BOT_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Hanzo Bot.app/Contents/MacOS/Hanzo Bot"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Hanzo Bot"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Hanzo Bot"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Hanzo Bot"
...
log "==> Killing existing Hanzo Bot instances"
kill_all_openclaw
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Hanzo Bot"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Hanzo Bot.app" ]]; then
    APP_BUNDLE="/Applications/Hanzo Bot.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Hanzo Bot.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Hanzo Bot.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Hanzo Bot.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set BOT_APP_BUNDLE to your installed Hanzo Bot.app"
}
