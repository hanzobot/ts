#!/usr/bin/env bash
# Reset HanzoBot like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${BOT_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="HanzoBot.app/Contents/MacOS/HanzoBot"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/HanzoBot"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/HanzoBot"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/HanzoBot"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.hanzo.bot.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/hanzo-bot-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${BOT_RESTART_LOG:-/tmp/hanzo-bot-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${BOT_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.hanzo-bot/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  node hanzo-bot.mjs daemon install --force --runtime node"
      log "  node hanzo-bot.mjs daemon restart"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.hanzo.bot.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing HanzoBot instances"
kill_all_hanzo-bot
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product HanzoBot"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/HanzoBot.app" ]]; then
    APP_BUNDLE="/Applications/HanzoBot.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/HanzoBot.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/HanzoBot.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/HanzoBot.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set BOT_APP_BUNDLE to your installed HanzoBot.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node hanzo-bot.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node hanzo-bot.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".hanzo-bot", "hanzo-bot.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: HanzoBot is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.hanzo.bot.gateway.plist' | head -n 40 || true"
fi
