...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/hanzo-bot/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "HanzoBot Re-Auth" -c "After getting key, run: ssh $SERVER '~/hanzo-bot/scripts/mobile-reauth.sh'" --id hanzo-bot-auth
        ;;
    *)
