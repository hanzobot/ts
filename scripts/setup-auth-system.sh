#!/bin/bash
# Setup HanzoBot Auth Management System
...
echo "=== HanzoBot Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'hanzo-bot-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  HanzoBot message: Send warning via HanzoBot itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/hanzo-bot-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/hanzo-bot-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/hanzo-bot-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now hanzo-bot-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status hanzo-bot-auth-monitor.timer"
echo ""
echo "Quick commands:"
