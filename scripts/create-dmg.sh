...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "Hanzo Bot")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "Hanzo Bot")
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/Hanzo Bot/Resources/Hanzo Bot.icns"
  if [[ -f "$ICON_SRC" ]]; then
