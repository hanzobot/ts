...
BOT_DOCKER_BUILD_USE_BUILDX="${BOT_DOCKER_BUILD_USE_BUILDX:-0}"
BOT_DOCKER_BUILD_CACHE_FROM="${BOT_DOCKER_BUILD_CACHE_FROM:-}"
BOT_DOCKER_BUILD_CACHE_TO="${BOT_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${BOT_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${BOT_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${BOT_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${BOT_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${BOT_DOCKER_BUILD_CACHE_TO}")
  fi
fi
