...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://hanzo.bot/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://hanzo.bot/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://hanzo.bot/abc-123", baseHref)).toBe(
      "blob:https://hanzo.bot/abc-123",
    );
  });
