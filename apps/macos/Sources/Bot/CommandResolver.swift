...
enum CommandResolver {
    private static let projectRootDefaultsKey = "bot.gatewayProjectRootPath"
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/hanzo-bot")
    ...
    static func projectHanzoBotExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("bot.mjs").path,
            root.appendingPathComponent("bin/hanzo-bot.js").path,
        ]
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/hanzo-bot"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/hanzo-bot.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/hanzo-bot.js"
            node "$PRJ/bin/hanzo-bot.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent hanzo-bot"
    ...
    {
        let root = configRoot ?? BotConfigFile.loadDict()
