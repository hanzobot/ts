    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("Allow these so Hanzo Bot can notify and capture when needed.")
                    .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = Hanzo BotLocationMode.off.rawValue
    ...
    private var locationMode: Hanzo BotLocationMode {
        Hanzo BotLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: Hanzo BotLocationMode) async -> Bool {
