...
actor GatewayEndpointStore {
    ...
    {
        let raw = env["BOT_GATEWAY_PASSWORD"] ?? ""
    ...
    init(deps: Deps = .live) {
        ...
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            root: Hanzo BotConfigFile.loadDict(),
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: Hanzo BotConfigFile.loadDict())
    ...
    {
        if let envBind = env["BOT_GATEWAY_BIND"] {
    ...
    private static func resolveGatewayScheme(
        ...
        env: [String: String]) -> String
    {
        if let envValue = env["BOT_GATEWAY_TLS"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !envValue.isEmpty
        {
