...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: Hanzo BotProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: Hanzo BotProtocol.AnyCodable] = [
            "presence": .init([Hanzo BotProtocol.AnyCodable(entry)]),
        ]
        let payload = Hanzo BotProtocol.AnyCodable(payloadMap)
