...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.hanzo.bot.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Hanzo Bot", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Hanzo Bot gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to Hanzo Bot.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "Hanzo BotShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Hanzo Bot is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Hanzo Bot Share",
