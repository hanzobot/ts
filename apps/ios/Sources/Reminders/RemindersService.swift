...
final class RemindersService: RemindersServicing {
    func list(params: Hanzo BotRemindersListParams) async throws -> Hanzo BotRemindersListPayload {
        ...
        let payload: [Hanzo BotReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return Hanzo BotReminderPayload(
        ...
        return Hanzo BotRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: Hanzo BotRemindersAddParams) async throws -> Hanzo BotRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = Hanzo BotReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return Hanzo BotRemindersAddPayload(reminder: payload)
    }
