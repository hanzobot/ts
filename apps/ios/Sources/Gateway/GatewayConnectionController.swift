...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [Hanzo BotCapability.canvas.rawValue, Hanzo BotCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(Hanzo BotCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(Hanzo BotCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = Hanzo BotLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(Hanzo BotCapability.location.rawValue) }

        caps.append(Hanzo BotCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(Hanzo BotCapability.watch.rawValue)
        }
        caps.append(Hanzo BotCapability.photos.rawValue)
        caps.append(Hanzo BotCapability.contacts.rawValue)
        caps.append(Hanzo BotCapability.calendar.rawValue)
        caps.append(Hanzo BotCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(Hanzo BotCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            Hanzo BotScreenCommand.record.rawValue,
            Hanzo BotSystemCommand.notify.rawValue,
            Hanzo BotChatCommand.push.rawValue,
        ...
        if caps.contains(Hanzo BotCapability.camera.rawValue) {
        ...
        if caps.contains(Hanzo BotCapability.location.rawValue) {
            commands.append(Hanzo BotLocationCommand.get.rawValue)
        }
        if caps.contains(Hanzo BotCapability.device.rawValue) {
            commands.append(Hanzo BotDeviceCommand.status.rawValue)
            commands.append(Hanzo BotDeviceCommand.info.rawValue)
        }
        if caps.contains(Hanzo BotCapability.watch.rawValue) {
            commands.append(Hanzo BotWatchCommand.status.rawValue)
            commands.append(Hanzo BotWatchCommand.notify.rawValue)
        }
        if caps.contains(Hanzo BotCapability.photos.rawValue) {
            commands.append(Hanzo BotPhotosCommand.latest.rawValue)
        }
        if caps.contains(Hanzo BotCapability.contacts.rawValue) {
            commands.append(Hanzo BotContactsCommand.search.rawValue)
            commands.append(Hanzo BotContactsCommand.add.rawValue)
        }
        if caps.contains(Hanzo BotCapability.calendar.rawValue) {
            commands.append(Hanzo BotCalendarCommand.events.rawValue)
            commands.append(Hanzo BotCalendarCommand.add.rawValue)
        }
        if caps.contains(Hanzo BotCapability.reminders.rawValue) {
            commands.append(Hanzo BotRemindersCommand.list.rawValue)
            commands.append(Hanzo BotRemindersCommand.add.rawValue)
        }
        if caps.contains(Hanzo BotCapability.motion.rawValue) {
            commands.append(Hanzo BotMotionCommand.activity.rawValue)
            commands.append(Hanzo BotMotionCommand.pedometer.rawValue)
        }
