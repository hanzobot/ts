...
final class TalkModeManager: NSObject {
    ...
    private var pttCompletion: CheckedContinuation<Hanzo BotTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = Hanzo BotTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> Hanzo BotTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return Hanzo BotTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> Hanzo BotTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> Hanzo BotTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return Hanzo BotTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> Hanzo BotTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = Hanzo BotTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: Hanzo BotTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: Hanzo BotAgentEventPayload.self
            ) else {
                continue
            }
