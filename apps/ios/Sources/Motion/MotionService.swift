...
final class MotionService: MotionServicing {
    func activities(params: Hanzo BotMotionActivityParams) async throws -> Hanzo BotMotionActivityPayload {
        ...
        let mapped: [Hanzo BotMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return Hanzo BotMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: Hanzo BotPedometerParams) async throws -> Hanzo BotPedometerPayload {
        ...
        let payload: Hanzo BotPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = Hanzo BotPedometerPayload(
