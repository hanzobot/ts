...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: Hanzo BotWatchNotifyParams) -> Hanzo BotWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [Hanzo BotWatchAction]?,
        kind: String?,
        promptId: String?) -> [Hanzo BotWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> Hanzo BotWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return Hanzo BotWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                Hanzo BotWatchAction(id: "approve", label: "Approve"),
                Hanzo BotWatchAction(id: "decline", label: "Decline", style: "destructive"),
                Hanzo BotWatchAction(id: "open_phone", label: "Open iPhone"),
                Hanzo BotWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            Hanzo BotWatchAction(id: "done", label: "Done"),
            Hanzo BotWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            Hanzo BotWatchAction(id: "open_phone", label: "Open iPhone"),
            Hanzo BotWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: Hanzo BotWatchRisk?,
        priority: Hanzo BotNotificationPriority?) -> Hanzo BotWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: Hanzo BotNotificationPriority?,
        risk: Hanzo BotWatchRisk?) -> Hanzo BotNotificationPriority?
    {
