...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> Hanzo BotDeviceStatusPayload {
        ...
        return Hanzo BotDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> Hanzo BotDeviceInfoPayload {
        ...
        return Hanzo BotDeviceInfoPayload(
    ...
    private func batteryStatus() -> Hanzo BotBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: Hanzo BotBatteryState = switch device.batteryState {
        ...
        return Hanzo BotBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> Hanzo BotThermalStatusPayload {
        let state: Hanzo BotThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return Hanzo BotThermalStatusPayload(state: state)
    }

    private func storageStatus() -> Hanzo BotStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return Hanzo BotStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
