...
final class ContactsService: ContactsServicing {
    ...
    func search(params: Hanzo BotContactsSearchParams) async throws -> Hanzo BotContactsSearchPayload {
        ...
        return Hanzo BotContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: Hanzo BotContactsAddParams) async throws -> Hanzo BotContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return Hanzo BotContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return Hanzo BotContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> Hanzo BotContactPayload {
        Hanzo BotContactPayload(
