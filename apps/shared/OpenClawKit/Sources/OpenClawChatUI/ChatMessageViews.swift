...
struct ChatMessageBubble: View {
    let message: Hanzo BotChatMessage
    let style: Hanzo BotChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? Hanzo BotChatTheme.userText : Hanzo BotChatTheme.assistantText
    ...
    private var inlineAttachments: [Hanzo BotChatMessageContent] {
    ...
    private var toolCalls: [Hanzo BotChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [Hanzo BotChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? Hanzo BotChatTheme.userBubble
        }
        if self.style == .onboarding {
            return Hanzo BotChatTheme.onboardingAssistantBubble
        }
        return Hanzo BotChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return Hanzo BotChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: Hanzo BotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? Hanzo BotChatTheme.userText : Hanzo BotChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: Hanzo BotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(Hanzo BotChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [Hanzo BotChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: Hanzo BotChatTheme.assistantText)
            }
        }
    }
}
