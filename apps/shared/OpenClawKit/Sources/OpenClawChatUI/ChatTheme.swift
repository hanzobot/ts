...
enum Hanzo BotChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("Hanzo BotChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("Hanzo BotChatTheme.onboardingAssistantBubble"),
...
enum Hanzo BotPlatformImageFactory {
    static func image(_ image: Hanzo BotPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
