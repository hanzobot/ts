...
val androidStoreFile = providers.gradleProperty("BOT_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("BOT_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("BOT_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("BOT_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set BOT_ANDROID_STORE_FILE, " +
            "BOT_ANDROID_STORE_PASSWORD, BOT_ANDROID_KEY_ALIAS, and " +
            "BOT_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/Hanzo BotKit/Sources/Hanzo BotKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.hanzo.bot.app"
        minSdk = 31
        targetSdk = 36
