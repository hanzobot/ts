...
- Canvas web content already supports live reload when loaded from Gateway `__bot__/canvas/` (see `docs/platforms/android.md`).
...
2) Android app is connected to that gateway and `hanzo-bot nodes status` shows it as paired + connected.
...
```bash
hanzo-bot devices list
hanzo-bot devices approve --latest
```
...
- `BOT_ANDROID_GATEWAY_URL=ws://...` (default: from your local Hanzo Bot config)
...
- `pairing required` before tests start:
  - approve pending device pairing (`hanzo-bot devices approve --latest`) and rerun.
