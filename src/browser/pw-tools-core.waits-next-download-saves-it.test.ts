...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredHanzoBotTmpDir: vi.fn(() => "/tmp/openclaw"),
}));
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredHanzoBotTmpDir.mockReturnValue("/tmp/openclaw");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredHanzoBotTmpDir.mockReturnValue("/tmp/openclaw-preferred");
    ...
    expect(tmpDirMocks.resolvePreferredHanzoBotTmpDir).toHaveBeenCalled();
  });
