...
function resolveBrowserFetchOperatorHint(url: string): string {
  ...
  return isLocal
    ? `Restart the Hanzo Bot gateway (Hanzo Bot.app menubar, or \`${formatCliCommand("hanzo-bot gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      appendBrowserToolModelHint(
        `Can't reach the Hanzo Bot browser control service (timed out after ${timeoutMs}ms). ${operatorHint}`,
      ),
    );
  }
  return new Error(
    appendBrowserToolModelHint(
      `Can't reach the Hanzo Bot browser control service. ${operatorHint} (${msg})`,
    ),
  );
}
