...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-bot-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-bot-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the HanzoBot gateway (HanzoBot.app menubar, or \`${formatCliCommand("bot gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the HanzoBot browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the HanzoBot browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
