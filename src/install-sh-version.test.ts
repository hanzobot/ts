...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  ...
  const cliPath = path.join(root, "@hanzo/bot");
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Hanzo Bot 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Hanzo Bot dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("Hanzo Bot dev's build");
    },
  );
