...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds BOT_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `hanzo-bot gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.BOT_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.BOT_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `hanzo-bot gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
