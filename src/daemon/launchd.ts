...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromEnv = parseStrictPositiveInteger(env.BOT_GATEWAY_PORT ?? "");
  return fromEnv ?? null;
}
...
function throwBootstrapGuiSessionError(params: {
  ...
  throw new Error(
    [
      ...
      "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.hanzo.bot/gateway",
    ].join("\n"),
  );
}
...
export async function installLaunchAgent({
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    plistPath,
    actionHint: "hanzo-bot gateway install --force",
  });
...
export async function restartLaunchAgent({
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    serviceTarget,
    plistPath,
    actionHint: "hanzo-bot gateway restart",
  });
