  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        ...
        BOT_PROFILE: "rescue",
        BOT_STATE_DIR: "C:\\State\\openclaw",
      },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Hanzo Bot Share\\node.exe" "\\\\fileserver\\Hanzo Bot Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Hanzo Bot Share\\node.exe",
            "\\\\fileserver\\Hanzo Bot Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
