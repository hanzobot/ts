...
export const GATEWAY_WINDOWS_TASK_NAME = "HanzoBot Gateway";
...
export const NODE_WINDOWS_TASK_NAME = "HanzoBot Node";
...
export function resolveGatewayWindowsTaskName(profile?: string): string {
  ...
  return `HanzoBot Gateway (${normalized})`;
}
...
export function formatGatewayServiceDescription(params?: {
  ...
  if (parts.length === 0) {
    return "HanzoBot Gateway";
  }
  return `HanzoBot Gateway (${parts.join(", ")})`;
}
...
export function formatNodeServiceDescription(params?: { version?: string }): string {
  const version = params?.version?.trim();
  if (!version) {
    return "HanzoBot Node Host";
  }
  return `HanzoBot Node Host (v${version})`;
}
