...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.bot.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.bot.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['bot agents set-identity --agent main --name "HanzoBot" --emoji "🦞"', "Set name + emoji."],
  ...
  [
    "bot agents set-identity --identity-file ~/.bot/workspace/IDENTITY.md --agent main",
