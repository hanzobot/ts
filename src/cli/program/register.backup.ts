...
export function registerBackupCommand(program: Command) {
  const backup = program
    .command("backup")
    .description("Create and verify local backup archives for Hanzo Bot state")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/backup", "docs.hanzo.bot/cli/backup")}\n`,
    );
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["hanzo-bot backup create", "Create a timestamped backup in the current directory."],
          [
            "hanzo-bot backup create --output ~/Backups",
            "Write the archive into an existing backup directory.",
          ],
          [
            "hanzo-bot backup create --dry-run --json",
            "Preview the archive plan without writing any files.",
          ],
          [
            "hanzo-bot backup create --verify",
            "Create the archive and immediately validate its manifest and payload layout.",
          ],
          [
            "hanzo-bot backup create --no-include-workspace",
            "Back up state/config without agent workspace files.",
          ],
          ["hanzo-bot backup create --only-config", "Back up only the active JSON config file."],
        ])}`,
    )
    .action(async (opts) => {
  ...
  backup
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "hanzo-bot backup verify ./2026-03-09T00-00-00.000Z-openclaw-backup.tar.gz",
            "Check that the archive structure and manifest are intact.",
          ],
          [
            "hanzo-bot backup verify ~/Backups/latest.tar.gz --json",
