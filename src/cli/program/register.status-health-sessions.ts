...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["hanzo-bot status", "Show channel health + session summary."],
          ["hanzo-bot status --all", "Full diagnosis (read-only)."],
          ["hanzo-bot status --json", "Machine-readable output."],
          ["hanzo-bot status --usage", "Show model provider usage/quota snapshots."],
          [
            "hanzo-bot status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["hanzo-bot status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["hanzo-bot sessions", "List all sessions."],
          ["hanzo-bot sessions --agent work", "List sessions for one agent."],
          ["hanzo-bot sessions --all-agents", "Aggregate sessions across agents."],
          ["hanzo-bot sessions --active 120", "Only last 2 hours."],
          ["hanzo-bot sessions --json", "Machine-readable output."],
          ["hanzo-bot sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["hanzo-bot sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "hanzo-bot sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["hanzo-bot sessions cleanup --enforce", "Apply maintenance now."],
          ["hanzo-bot sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["hanzo-bot sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "hanzo-bot sessions cleanup --enforce --store ./tmp/sessions.json",
