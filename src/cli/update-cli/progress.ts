...
const STEP_LABELS: Record<string, string> = {
  ...
  "hanzo-bot doctor entry": "Checking doctor entrypoint",
  "hanzo-bot doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g @hanzo/bot@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g @hanzo/bot@latest --omit=optional");
  }
