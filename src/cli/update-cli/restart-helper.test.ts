...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.hanzo.bot.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.hanzo.bot.gateway'");
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "Hanzo Bot Gateway"');
      expect(content).toContain('schtasks /Run /TN "Hanzo Bot Gateway"');
    ...
    it("uses BOT_WINDOWS_TASK_NAME override on Windows", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        BOT_PROFILE: "default",
        BOT_WINDOWS_TASK_NAME: "Hanzo Bot Gateway (custom)",
      });
      expect(content).toContain('schtasks /End /TN "Hanzo Bot Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "Hanzo Bot Gateway (custom)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      expect(content).toContain("gui/502/ai.hanzo.bot.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "Hanzo Bot Gateway (production)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        BOT_LAUNCHD_LABEL: "ai.hanzo.bot.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.hanzo.bot.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
