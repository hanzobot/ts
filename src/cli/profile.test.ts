import path from "node:path";
import { describe, expect, it } from "vitest";
import { formatCliCommand } from "./command-format.js";
import { applyCliProfileEnv, parseCliProfileArgs } from "./profile.js";

describe("parseCliProfileArgs", () => {
  it("leaves gateway --dev for subcommands", () => {
    const res = parseCliProfileArgs([
      "node",
      "@hanzo/bot",
      "gateway",
      "--dev",
      "--allow-unconfigured",
    ]);
    if (!res.ok) {
      throw new Error(res.error);
    }
    expect(res.profile).toBeNull();
    expect(res.argv).toEqual(["node", "@hanzo/bot", "gateway", "--dev", "--allow-unconfigured"]);
  });

  it("still accepts global --dev before subcommand", () => {
    const res = parseCliProfileArgs(["node", "@hanzo/bot", "--dev", "gateway"]);
    if (!res.ok) {
      throw new Error(res.error);
    }
    expect(res.profile).toBe("dev");
    expect(res.argv).toEqual(["node", "@hanzo/bot", "gateway"]);
  });

  it("parses --profile value and strips it", () => {
    const res = parseCliProfileArgs(["node", "@hanzo/bot", "--profile", "work", "status"]);
    if (!res.ok) {
      throw new Error(res.error);
    }
    expect(res.profile).toBe("work");
    expect(res.argv).toEqual(["node", "@hanzo/bot", "status"]);
  });

  it("rejects missing profile value", () => {
    const res = parseCliProfileArgs(["node", "@hanzo/bot", "--profile"]);
    expect(res.ok).toBe(false);
  });

  it.each([
    ["--dev first", ["node", "@hanzo/bot", "--dev", "--profile", "work", "status"]],
    ["--profile first", ["node", "@hanzo/bot", "--profile", "work", "--dev", "status"]],
  ])("rejects combining --dev with --profile (%s)", (_name, argv) => {
    const res = parseCliProfileArgs(argv);
    expect(res.ok).toBe(false);
  });
});

describe("applyCliProfileEnv", () => {
  it("fills env defaults for dev profile", () => {
    const env: Record<string, string | undefined> = {};
    applyCliProfileEnv({
      profile: "dev",
      env,
      homedir: () => "/home/peter",
    });
    const expectedStateDir = path.join(path.resolve("/home/peter"), ".openclaw-dev");
    expect(env.BOT_PROFILE).toBe("dev");
    expect(env.BOT_STATE_DIR).toBe(expectedStateDir);
    expect(env.BOT_CONFIG_PATH).toBe(path.join(expectedStateDir, "openclaw.json"));
    expect(env.BOT_GATEWAY_PORT).toBe("19001");
  });

  it("does not override explicit env values", () => {
    const env: Record<string, string | undefined> = {
      BOT_STATE_DIR: "/custom",
      BOT_GATEWAY_PORT: "19099",
    };
    applyCliProfileEnv({
      profile: "dev",
      env,
      homedir: () => "/home/peter",
    });
    expect(env.BOT_STATE_DIR).toBe("/custom");
    expect(env.BOT_GATEWAY_PORT).toBe("19099");
    expect(env.BOT_CONFIG_PATH).toBe(path.join("/custom", "openclaw.json"));
  });

  it("uses BOT_HOME when deriving profile state dir", () => {
    const env: Record<string, string | undefined> = {
      BOT_HOME: "/srv/openclaw-home",
      HOME: "/home/other",
    };
    applyCliProfileEnv({
      profile: "work",
      env,
      homedir: () => "/home/fallback",
    });

    const resolvedHome = path.resolve("/srv/openclaw-home");
    expect(env.BOT_STATE_DIR).toBe(path.join(resolvedHome, ".openclaw-work"));
    expect(env.BOT_CONFIG_PATH).toBe(
      path.join(resolvedHome, ".openclaw-work", "openclaw.json"),
    );
  });
});

describe("formatCliCommand", () => {
  it.each([
    {
      name: "no profile is set",
      cmd: "hanzo-bot doctor --fix",
      env: {},
      expected: "hanzo-bot doctor --fix",
    },
    {
      name: "profile is default",
      cmd: "hanzo-bot doctor --fix",
      env: { BOT_PROFILE: "default" },
      expected: "hanzo-bot doctor --fix",
    },
    {
      name: "profile is Default (case-insensitive)",
      cmd: "hanzo-bot doctor --fix",
      env: { BOT_PROFILE: "Default" },
      expected: "hanzo-bot doctor --fix",
    },
    {
      name: "profile is invalid",
      cmd: "hanzo-bot doctor --fix",
      env: { BOT_PROFILE: "bad profile" },
      expected: "hanzo-bot doctor --fix",
    },
    {
      name: "--profile is already present",
      cmd: "hanzo-bot --profile work doctor --fix",
      env: { BOT_PROFILE: "work" },
      expected: "hanzo-bot --profile work doctor --fix",
    },
    {
      name: "--dev is already present",
      cmd: "hanzo-bot --dev doctor",
      env: { BOT_PROFILE: "dev" },
      expected: "hanzo-bot --dev doctor",
    },
  ])("returns command unchanged when $name", ({ cmd, env, expected }) => {
    expect(formatCliCommand(cmd, env)).toBe(expected);
  });

  it("inserts --profile flag when profile is set", () => {
    expect(formatCliCommand("hanzo-bot doctor --fix", { BOT_PROFILE: "work" })).toBe(
      "hanzo-bot --profile work doctor --fix",
    );
  });

  it("trims whitespace from profile", () => {
    expect(formatCliCommand("hanzo-bot doctor --fix", { BOT_PROFILE: "  jbopenclaw  " })).toBe(
      "hanzo-bot --profile jbopenclaw doctor --fix",
    );
  });

  it("handles command with no args after openclaw", () => {
    expect(formatCliCommand("@hanzo/bot", { BOT_PROFILE: "test" })).toBe(
      "hanzo-bot --profile test",
    );
  });

  it("handles pnpm wrapper", () => {
    expect(formatCliCommand("pnpm hanzo-bot doctor", { BOT_PROFILE: "work" })).toBe(
      "pnpm hanzo-bot --profile work doctor",
    );
  });
});
