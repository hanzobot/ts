...
import type { BotConfig, ConfigFileSnapshot } from "../config/types.bot.js";
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/bot-update-tests";
  let fixtureCount = 0;
  ...
  beforeEach(() => {
    ...
    prepareRestartScript.mockResolvedValue("/tmp/bot-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "bot-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("HanzoBot update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as BotConfig,
        });
      },
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("bot-updated-root");
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("bot-update-wizard");
