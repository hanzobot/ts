...
const CONFIG_SET_EXAMPLE_REF = formatCliCommand(
  "hanzo-bot config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN",
);
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "hanzo-bot config set secrets.providers.vault --provider-source file --provider-path /etc/openclaw/secrets.json --provider-mode json",
);
const CONFIG_SET_EXAMPLE_BATCH = formatCliCommand(
  "hanzo-bot config set --batch-file ./config-set.batch.json --dry-run",
);
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("hanzo-bot doctor")}\` ${message}`;
}
