    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["hanzo-bot channels list", "List configured channels and auth profiles."],
          ["hanzo-bot channels status --probe", "Run channel status checks and probes."],
          [
            "hanzo-bot channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["hanzo-bot channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.hanzo.bot/cli/channels",
        )}\n`,
    );
