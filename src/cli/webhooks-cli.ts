...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.bot.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + HanzoBot hooks")
    ...
    .option("--hook-url <url>", "HanzoBot hook URL")
    .option("--hook-token <token>", "HanzoBot hook token")
