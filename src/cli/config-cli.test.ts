...
/**
 * Test for issue #6070:
 * `hanzo-bot config set/unset` must update snapshot.resolved (user config after $include/${ENV},
...
describe("config cli", () => {
  ...
  describe("config set parsing flags", () => {
    ...
    it("shows --strict-json and keeps --json as a legacy alias in help", async () => {
      ...
      expect(helpText).toContain("hanzo-bot config set gateway.port 19001 --strict-json");
      expect(helpText).toContain(
        "hanzo-bot config set channels.discord.token --ref-provider default --ref-source",
      );
