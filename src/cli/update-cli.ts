...
export function registerUpdateCli(program: Command) {
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["hanzo-bot update", "Update a source checkout (git)"],
        ["hanzo-bot update --channel beta", "Switch to beta channel (git + npm)"],
        ["hanzo-bot update --channel dev", "Switch to dev channel (git + npm)"],
        ["hanzo-bot update --tag beta", "One-off update to a dist-tag or version"],
        ["hanzo-bot update --tag main", "One-off package install from GitHub main"],
        ["hanzo-bot update --dry-run", "Preview actions without changing anything"],
        ["hanzo-bot update --no-restart", "Update without restarting the service"],
        ["hanzo-bot update --json", "Output result as JSON"],
        ["hanzo-bot update --yes", "Non-interactive (accept downgrade prompts)"],
        ["hanzo-bot update wizard", "Interactive update wizard"],
        ["hanzo-bot --update", "Shorthand for hanzo-bot update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run hanzo-bot update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.hanzo.bot/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["hanzo-bot update status", "Show channel + version status."],
          ["hanzo-bot update status --json", "JSON output."],
          ["hanzo-bot update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
