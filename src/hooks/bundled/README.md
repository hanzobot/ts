...
This directory contains hooks that ship with Hanzo Bot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
homepage: https://docs.hanzo.ai/automation/hooks#my-hook
metadata:
...
Hooks can be configured in `~/.hanzoai/bot.json`:
...
2. Restart gateway: `pkill -9 -f 'hanzo-bot.*gateway' && pnpm hanzo-bot gateway`
3. Enable the hook: `hanzo-bot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.hanzo.ai/automation/hooks
