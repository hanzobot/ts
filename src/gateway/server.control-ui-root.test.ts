...
async function withGlobalControlUiHardlinkFixture<T>(run: (rootPath: string) => Promise<T>) {
  ...
  try {
    const packageRoot = path.join(tmp, "pnpm-global", "5", "node_modules", "@hanzo/bot");
...
describe("gateway.controlUi.root", () => {
  test("rejects hardlinked index.html when configured root points at global Hanzo Bot package control-ui", async () => {
