...
export function listAgentsForGateway(cfg: Hanzo BotConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: Hanzo BotConfig): string {
...
function resolveSessionStoreAgentId(cfg: Hanzo BotConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: Hanzo BotConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: Hanzo BotConfig): GatewaySessionsDefaults {
