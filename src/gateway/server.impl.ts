...
function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("hanzo-bot doctor")}" to repair, then retry.`
    : "";
...
async function prepareGatewayStartupConfig(params: {
  ...
  ) => Promise<{ config: Hanzo BotConfig }>;
...
export async function startGatewayServer(
  ...
  let cfgAtStart: Hanzo BotConfig;
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `hanzo-bot config set gateway.auth.mode token` and `hanzo-bot config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  const transcriptUnsub = minimalTestGateway
    ...
    : onSessionTranscriptUpdate((update) => {
        ...
        const sessionSnapshot = sessionRow
          ...
          : {};
        const message = attachHanzo BotTranscriptMeta(update.message, {
