...
export function createAgentEventHandler({
  ...
  return (evt: AgentEventPayload) => {
    ...
    if (lifecyclePhase === "end" || lifecyclePhase === "error") {
      ...
      // Bot wallet deduction — fires after every agent run with usage data.
      // Works for both local runs and cloud agent pods (usage is included in event).
      if (lifecyclePhase === "end" && sessionKey && evt.data?.usage) {
        void deductWalletFromLifecycleEnd(sessionKey, evt.data);
      }
    }
  };
}

/** Extract bot ID from session key (format: "agent:botId:rest" or "botId:rest"). */
function extractBotIdFromSessionKey(sessionKey: string): string {
  const parts = sessionKey.split(":");
  // Cloud bot format: "agent:cloud-xxxx:main" → index 1
  if (parts.length >= 2 && parts[0] === "agent") {
    return parts[1] ?? "";
  }
  // Other formats: first non-empty part
  return parts[1] ?? parts[0] ?? "";
}

/** Deduct LLM usage from bot wallet after a lifecycle "end" event with usage data. */
async function deductWalletFromLifecycleEnd(
  sessionKey: string,
  ...
  try {
    ...
    const usage = eventData.usage as {
      input?: number;
      output?: number;
      cacheRead?: number;
      cacheWrite?: number;
    ...
    const provider = typeof eventData.provider === "string" ? eventData.provider : "";

    // Resolve model pricing from bot config, with Anthropic fallback.
    const { resolveModelCostConfig, estimateUsageCost } = await import("../utils/usage-format.js");
    ...
    const { deductWalletUsage } = await import("./billing/iam-billing-client.js");
    ...
    // Best-effort — never block the event flow
  }
}
