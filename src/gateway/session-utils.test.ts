...
function createSingleAgentAvatarConfig(workspace: string): BotConfig {
  return {
...
function createLegacyRuntimeListConfig(models?: Record<string, Record<string, never>>): BotConfig {
...
describe("gateway session utils", () => {
  ...
  test("listAgentsForGateway keeps explicit agents.list scope over disk-only agents (scope boundary)", async () => {
    await withStateDirEnv("bot-agent-list-scope-", async ({ stateDir }) => {
...
describe("loadCombinedSessionStoreForGateway includes disk-only agents (#32804)", () => {
  test("ACP agent sessions are visible even when agents.list is configured", async () => {
    await withStateDirEnv("bot-acp-vis-", async ({ stateDir }) => {
