...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: BotConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: BotConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: BotConfig) => string[];
  isConfigured?: (account: unknown, cfg: BotConfig) => boolean;
  isEnabled?: (account: unknown, cfg: BotConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
`,
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: BotConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: BotConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): BotConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain("~/.bot/credentials/discord-allowFrom.json:team.owner");
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<BotConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: BotConfig;
      expectedSeverity: "warn" | "critical";
