 ...
 * Default: ~/.hanzoai/bot.json (or $BOT_STATE_DIR/openclaw.json)
 */
...
export function resolveDefaultConfigCandidates(
  ...
  const openclawStateDir = env.BOT_STATE_DIR?.trim() || env.BOT_STATE_DIR?.trim();
  if (openclawStateDir) {
...
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  ...
  const suffix = uid != null ? `openclaw-${uid}` : "@hanzo/bot";
