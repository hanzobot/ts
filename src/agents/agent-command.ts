...
function prependInternalEventContext(
  ...
  if (body.includes("Hanzo Bot runtime context (internal):")) {
    return body;
  }
...
async function prepareAgentCommandExecution(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("hanzo-bot agents list")}" to see configured agents.`,
      );
    }
  }
