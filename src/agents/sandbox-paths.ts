...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const hanzoBotTmpDir = path.resolve(resolvePreferredHanzoBotTmpDir());
  if (!isPathInside(hanzoBotTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: hanzoBotTmpDir });
  return resolved;
}
