...
function resolveBrowserBaseUrl(params: {
  ...
  if (!resolved.enabled) {
    throw new Error("Browser control is disabled. Set browser.enabled=true in ~/.bot/bot.json.");
  }
  return undefined;
}
...
export function createBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via HanzoBot's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      ...
      "Chrome extension relay needs an attached tab: user must click the HanzoBot Browser Relay toolbar icon on the tab (badge ON). If no tab is connected, ask them to attach it.",
