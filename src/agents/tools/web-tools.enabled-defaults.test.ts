...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
      ...
      if (idx === 1) {
        return new Response(
          JSON.stringify({
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "bot" }),
                      },
                    },
                  ],
                },
            ...
            search_results: [{ title: "HanzoBot", url: "https://bot.ai/docs", content: "docs" }],
          }),
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://bot.ai/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://bot.ai/docs"]);
