...
describe("resolveSandboxedMediaSource", () => {
  ...
  it("rejects symlinked Hanzo Bot tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Hanzo Bot tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  it("rejects symlinked Hanzo Bot tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInHanzo BotTmp(
