...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for startup commands that need model metadata", async () => {
    ...
    try {
      for (const scenario of [
        {
          argv: ["node", "@hanzo/bot", "--profile", "--", "config", "validate"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "@hanzo/bot", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        {
          argv: ["node", "@hanzo/bot", "gateway", "status", "--json"],
          expectedCalls: 0,
        },
      ]) {
        vi.resetModules();
  ...
  it("retries config loading after backoff when an initial load fails", async () => {
    ...
    const argvSnapshot = process.argv;
    process.argv = ["node", "@hanzo/bot", "config", "validate"];
    try {
      const { lookupContextTokens } = await import("./context.js");
