...
describe("buildAgentSystemPrompt", () => {
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- agents_list: List Hanzo Bot agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Hanzo Bot docs: /tmp/openclaw/docs");
    expect(prompt).toContain(
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`hanzo-bot ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for Hanzo Bot subagents");
