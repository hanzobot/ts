...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; Hanzo Bot handles all routing internally.",
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running Hanzo Bot process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List Hanzo Bot agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List Hanzo Bot agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside Hanzo Bot.";
  }

  const lines = [
    "You are a personal assistant running inside Hanzo Bot.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control Hanzo Bot's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "",
    ...safetySection,
    "## Hanzo Bot CLI Quick Reference",
    "Hanzo Bot is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- hanzo-bot gateway status",
    "- hanzo-bot gateway start",
    "- hanzo-bot gateway stop",
    "- hanzo-bot gateway restart",
    "If unsure, ask the user to run `hanzo-bot help` (or `hanzo-bot gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## Hanzo Bot Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, Hanzo Bot pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by Hanzo Bot and included below in Project Context.",
    "",
  ...
  if (!isMinimal) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'Hanzo Bot treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
