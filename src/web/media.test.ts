...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(resolvePreferredBotTmpDir(), "bot-media-test-"));
...
describe("web media loading", () => {
  beforeAll(() => {
    ...
    // Also keep it outside the HanzoBot temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["BOT_STATE_DIR"]);
    process.env.BOT_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "bot-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredBotTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default HanzoBot state workspace and sandbox roots", async () => {
  ...
  it("rejects default HanzoBot state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
