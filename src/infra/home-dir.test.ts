...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers BOT_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for BOT_HOME", () => {
    const result = resolveRequiredHomeDir(
      { BOT_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when BOT_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ BOT_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("resolveOsHomeDir", () => {
  it("ignores BOT_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          BOT_HOME: "/srv/openclaw-home",
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of BOT_HOME", () => {
    expect(
