...
const loadHanzo BotPluginsMock = vi.hoisted(() => vi.fn());
...
describe("outbound channel resolution", () => {
  ...
  it("returns the already-registered plugin without bootstrapping", async () => {
    ...
    expect(loadHanzo BotPluginsMock).not.toHaveBeenCalled();
  });
  ...
  it("bootstraps plugins once per registry key and returns the newly loaded plugin", async () => {
    ...
    expect(loadHanzo BotPluginsMock).toHaveBeenCalledTimes(1);
    expect(loadHanzo BotPluginsMock).toHaveBeenLastCalledWith({
      config: { autoEnabled: true },
      workspaceDir: "/tmp/workspace",
      runtimeOptions: {
        allowGatewaySubagentBinding: true,
  ...
  it("retries bootstrap after a transient load failure", async () => {
    getChannelPluginMock.mockReturnValue(undefined);
    loadHanzo BotPluginsMock.mockImplementationOnce(() => {
    ...
    expect(loadHanzo BotPluginsMock).toHaveBeenCalledTimes(2);
  });
});
