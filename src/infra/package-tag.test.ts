...
describe("normalizePackageTagInput", () => {
  const packageNames = ["@hanzo/bot", "@hanzo/bot-plugin"] as const;
  ...
  it("strips known package-name prefixes before returning the tag", () => {
    ...
    expect(normalizePackageTagInput("@hanzo/bot-plugin@2026.2.24", packageNames)).toBe("2026.2.24");
    expect(normalizePackageTagInput("openclaw@   ", packageNames)).toBeNull();
  });

  it("treats exact known package names as an empty tag", () => {
    expect(normalizePackageTagInput("@hanzo/bot", packageNames)).toBeNull();
    expect(normalizePackageTagInput(" @hanzo/bot-plugin ", packageNames)).toBeNull();
  });
