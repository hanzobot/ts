...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      ...
      serial: 2025121701,
      gatewayPort: 18789,
      displayName: "Mac Studio (HanzoBot)",
    ...
    expect(txt).toContain(`_hanzo-bot-gw._tcp IN PTR studio-london._hanzo-bot-gw._tcp`);
    expect(txt).toContain(`studio-london._hanzo-bot-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (HanzoBot)`);
