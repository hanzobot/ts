...
export const POSIX_BOT_TMP_DIR = "/tmp/openclaw";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredHanzo BotTmpDirOptions = {
...
export function resolvePreferredHanzo BotTmpDir(
  options: ResolvePreferredHanzo BotTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "@hanzo/bot" : `openclaw-${uid}`;
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Hanzo Bot temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Hanzo Bot temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_BOT_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_BOT_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_BOT_TMP_DIR)) {
      return POSIX_BOT_TMP_DIR;
    }
  ...
  try {
    ...
    chmodSync(POSIX_BOT_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_BOT_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_BOT_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_BOT_TMP_DIR;
