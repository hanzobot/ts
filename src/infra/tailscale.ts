...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.BOT_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Hanzo Bot. You can keep running the web gateway without it: \`${formatCliCommand("hanzo-bot gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
