...
describe("ports helpers", () => {
  ...
  it("prints an HanzoBot-specific hint when port details look like another HanzoBot instance", async () => {
    const runtime = {
      error: vi.fn(),
      log: vi.fn(),
      exit: vi.fn() as unknown as (code: number) => never,
    ...
    const messages = runtime.error.mock.calls.map((call) => stripAnsi(String(call[0] ?? "")));
    expect(messages.join("\n")).toContain("another HanzoBot instance is already running");
  });
  ...
  it("classifies ssh and gateway listeners", () => {
    ...
    expect(
      classifyPortListener(
        {
          commandLine: "node /Users/me/Projects/bot/dist/entry.js gateway",
        },
        18789,
      ),
    ).toBe("gateway");
