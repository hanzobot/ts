...
function getSecretProviders(config: BotConfig): Record<string, SecretProviderConfig> {
...
export function buildConfigureCandidates(config: BotConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  config: BotConfig;
  authoredBotConfig?: BotConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredBotConfig ?? params.config;
