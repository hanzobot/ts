...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_BOT_EXTENSIONS: "missing_bot_extensions",
  EMPTY_BOT_EXTENSIONS: "empty_bot_extensions",
...
async function installPluginFromPackageDir(
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "bot security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "bot security audit --deep" after install.`,
    );
  }
