...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Hanzo Bot uses this to expose
 ...
 *
 * Runs after Hanzo Bot resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Hanzo Bot auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Hanzo Bot wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Hanzo Bot builds its generic
...
/**
 * Provider-owned stream wrapper hook after Hanzo Bot applies its generic
 ...
 *
 * Return `true` or `false` to override Hanzo Bot's built-in provider cache TTL
 ...
 *
 * Runs only after Hanzo Bot exhausts normal env/profile/config auth resolution
 ...
 *
 * Runs after Hanzo Bot loads the discovered model catalog and merges configured
...
export type ProviderPlugin = {
   ...
   *
   * Hanzo Bot calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic Hanzo Bot wrappers.
   *
   ...
   *
   * Called after Hanzo Bot resolves the raw configured credential but before the
   ...
   * hint than Hanzo Bot's generic auth-store guidance.
   */
   ...
   * `errorMessage` when Hanzo Bot should surface a provider-specific hint for
   ...
   * merging. Hanzo Bot deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * Hanzo Bot uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `hanzo-bot doctor`.
   *
   ...
   *
   * Hanzo Bot calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Hanzo Bot's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type PluginFormat = "@hanzo/bot" | "bundle";
