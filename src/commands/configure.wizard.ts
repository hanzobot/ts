...
async function runGatewayHealthCheck(params: {
  ...
  const token =
    process.env.BOT_GATEWAY_TOKEN ?? process.env.BOT_GATEWAY_TOKEN ?? configuredToken;
  const password =
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Hanzo Bot update wizard" : "Hanzo Bot configure");
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("hanzo-bot doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
