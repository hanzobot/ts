...
function resolveAgentName(cfg: HanzoBotConfig, agentId: string) {
...
function resolveAgentModel(cfg: HanzoBotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: HanzoBotConfig): AgentSummary[] {
