...
function resolveMode(cfg: BotConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: BotConfig = configResult.cfg;
  ...
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("bot config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("bot config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
