...
function resolveMode(cfg: Hanzo BotConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: Hanzo BotConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("hanzo-bot configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("hanzo-bot config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("hanzo-bot setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("hanzo-bot config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("hanzo-bot config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("hanzo-bot doctor --fix")}" to apply changes.`);
  }
