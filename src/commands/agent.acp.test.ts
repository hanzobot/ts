...
function createAcpEnabledConfig(home: string, storePath: string): BotConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
  ...
  it.each([
    {
      ...
      acpOverrides: { enabled: false } satisfies Partial<NonNullable<BotConfig["acp"]>>,
    },
    {
      name: "blocks ACP turns when ACP dispatch is disabled by policy",
      acpOverrides: {
        dispatch: { enabled: false },
      } satisfies Partial<NonNullable<BotConfig["acp"]>>,
    },
