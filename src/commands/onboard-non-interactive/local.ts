        ...
        hints:
          daemonInstall.skippedReason === "systemd-user-unavailable"
            ? [
                ...
                "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `hanzo-bot gateway run` or `hanzo-bot agent --local`.",
              ]
            : [`Run \`${formatCliCommand("hanzo-bot gateway status --deep")}\` for more detail.`],
      });
      runtime.exit(1);
      return;
    }
      ...
      logNonInteractiveOnboardingFailure({
        ...
        hints: !opts.installDaemon
          ? [
              ...
              `Fix: start \`${formatCliCommand("hanzo-bot gateway run")}\`, re-run with \`--install-daemon\`, or use \`--skip-health\`.`,
