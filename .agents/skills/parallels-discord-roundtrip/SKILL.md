...
```bash
export BOT_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.hanzoai/bot.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env BOT_PARALLELS_DISCORD_TOKEN \
...
- Use the `openclaw` wrapper for guest `message send/read`; `node hanzo-bot.mjs message ...` does not expose the lazy message subcommands the same way.
...
- host inbound nonce appears in `hanzo-bot message read` output
