# Hanzo Bot ACP Bridge

This document describes how the Hanzo Bot ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`hanzo-bot acp` exposes an ACP agent over stdio and forwards prompts to a running
Hanzo Bot Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Hanzo Bot Gateway
...
- ACP client spawns `hanzo-bot acp` and speaks ACP messages over stdio.
...
`hanzo-bot acp` resolves the Gateway URL and auth from CLI flags or config:
